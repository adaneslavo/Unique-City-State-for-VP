...
function GetCityStateStatusText(majorPlayerID, minorPlayerID)
	...
	if majorPlayer and minorPlayer then
		...
		elseif minorPlayer:GetPeaceBlockedTurns(majorTeamID) > 0 then		-- Peace blocked due to attacking too recently
			strStatusText = "[COLOR_NEGATIVE_TEXT]" .. L("TXT_KEY_PEACE_BLOCKED_TURNS", minorPlayer:GetPeaceBlockedTurns(iActiveTeam))
...
function GetCityStateBonuses(majorPlayerID, minorPlayerID)
	...
	if minorPlayer then
		...
		local happinessBonus = minorPlayer:GetMinorCivCurrentHappinessBonus(majorPlayerID)
		
		if happinessBonus ~= 0 then
			table_insert(tips, L("TXT_KEY_CSTATE_HAPPINESS_BONUS", happinessBonus))
		end
...
function GetCityStateStatusToolTip(majorPlayerID, minorPlayerID, isFullInfo)
	...
	if majorPlayer and minorPlayer then
		...
		for resource in GameInfo.Resources() do
			if resource.ResourceClassType ~= "RESOURCECLASS_BONUS" then
				local resourceForExport = minorPlayer:GetResourceExport(resource.ID) + minorPlayer:GetNumResourceAvailable(resource.ID, false)
		...
		if eMinorPersonality == eMinorPersonalityFriendly then
			...
			sMinorPersonalityStr = L("TXT_KEY_UCS_HELPER_HEADER_PERSONALITY_FRIENDLY")
		elseif eMinorPersonality == eMinorPersonalityNeutral then
			sMinorPersonalityStrCap = "NEUTRAL"
			sMinorPersonalityStr = L("TXT_KEY_UCS_HELPER_HEADER_PERSONALITY_NEUTRAL")
		...
		if minorPlayer:IsMinorCivHasUniqueUnit() then
			...
			local unitSQL = GameInfo.Units{ID=eUniqueUnit}()
			local sUniqueUnitName = L(unitSQL.Description)
		...
		if influenceAccumulated ~= influenceAnchor then
			table_insert(tips, newLine .. L("TXT_KEY_CSTATE_INFLUENCE_RATE", minorPlayer:GetFriendshipChangePerTurnTimes100(majorPlayerID) / 100, influenceAnchor))
		end
		
		-- Bullying
		if minorPlayer:CanMajorBullyGold(majorPlayerID) then
			table_insert(tips, newLine .. L("TXT_KEY_CSTATE_CAN_BULLY", minorPlayer:GetMajorBullyValue(majorPlayerID)))
		else
			table_insert(tips, newLine .. L("TXT_KEY_CSTATE_CANNOT_BULLY", minorPlayer:GetMajorBullyValue(majorPlayerID)))
		end
		...
		if isFullInfo then
			...
			if #bonuses > 0 then
				table_insert(tips, newLine .. table_concat(bonuses, newLine))
			end
			...
			if #protectors > 0 or #wars > 0 then
				table_insert(tips, "")
				
				if #protectors > 0 then
					table_insert(tips, L("TXT_KEY_UCS_HELPER_PROTECTED", table_concat(protectors, ", ")))
				end
				
				if #wars > 0 then
					table_insert(tips, L("TXT_KEY_UCS_HELPER_AT_WAR", table_concat(wars, ", ")))
				end
			end
		end
