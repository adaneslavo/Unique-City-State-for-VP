...
SELECT		'BUILDINGCLASS_DUMMY_UCS_MW'
WHERE EXISTS (SELECT * FROM Resources WHERE Type='RESOURCE_TROPICAL_FISH');

CREATE TRIGGER IF NOT EXISTS UCSCompatibilityMW
AFTER INSERT ON Resources
WHEN NEW.Type = 'RESOURCE_TROPICAL_FISH'
BEGIN
    INSERT INTO BuildingClasses (Type)
	VALUES		('BUILDINGCLASS_DUMMY_UCS_MW');
END;


/* Basques (Irunea) */
INSERT INTO BuildingClasses (Type)
SELECT		'BUILDINGCLASS_DUMMY_UCS_BASQUE'
WHERE EXISTS (SELECT * FROM Civilizations WHERE Type='CIVILIZATION_GT_BASQUES');

CREATE TRIGGER IF NOT EXISTS UCSCompatibilityBasques
AFTER INSERT ON Civilizations
WHEN NEW.Type = 'CIVILIZATION_GT_BASQUES'
BEGIN
    INSERT INTO BuildingClasses (Type)
	VALUES		('BUILDINGCLASS_DUMMY_UCS_BASQUE');
END;

/* Nan Madol */
INSERT INTO BuildingClasses (Type)
SELECT		'BUILDINGCLASS_DUMMY_UCS_NAN_MADOL'
WHERE EXISTS (SELECT * FROM Civilizations WHERE Type='CIVILIZATION_JAR_NANMADOL');

CREATE TRIGGER IF NOT EXISTS UCSCompatibilityNanMadol
AFTER INSERT ON Civilizations
WHEN NEW.Type = 'CIVILIZATION_JAR_NANMADOL'
BEGIN
    INSERT INTO BuildingClasses (Type)
	VALUES		('BUILDINGCLASS_DUMMY_UCS_NAN_MADOL');
END;

/* Marajoara (Camutins) */
INSERT INTO BuildingClasses (Type)
SELECT		'BUILDINGCLASS_DUMMY_UCS_MARAJOARA'
WHERE EXISTS (SELECT * FROM Civilizations WHERE Type='CIVILIZATION_JAR_MARAJOARA');

CREATE TRIGGER IF NOT EXISTS UCSCompatibilityMarajoara
AFTER INSERT ON Civilizations
WHEN NEW.Type = 'CIVILIZATION_JAR_MARAJOARA'
BEGIN
    INSERT INTO BuildingClasses (Type)
	VALUES		('BUILDINGCLASS_DUMMY_UCS_MARAJOARA');
END;
--==========================================================================================================================
-- UPDATES AND DEFINES
--==========================================================================================================================	
...
---------------------------------------------------
-- Updates - Blocking of added CS by VP 5.0
---------------------------------------------------
UPDATE MinorCivilizations SET Playable = 0 
WHERE Type IN ('MINOR_CIV_COPENHAGEN', 'MINOR_CIV_DUBLIN', 'MINOR_CIV_EDINBURGH', 'MINOR_CIV_HELSINKI', 'MINOR_CIV_JAKARTA', 
'MINOR_CIV_LISBON', 'MINOR_CIV_MARRAKECH', 'MINOR_CIV_OSLO', 'MINOR_CIV_RIO_DE_JANEIRO', 'MINOR_CIV_SEOUL', 'MINOR_CIV_STOCKHOLM', 
'MINOR_CIV_VENICE', 'MINOR_CIV_VIENNA', 'MINOR_CIV_WARSAW', 'MINOR_CIV_BORNU', 'MINOR_CIV_BUNKEYA', 'MINOR_CIV_LUBA', 'MINOR_CIV_OUAGADOUGOU', 
'MINOR_CIV_SEGOU', 'MINOR_CIV_SOKOTO', 'MINOR_CIV_ABERNETHY', 'MINOR_CIV_AKSUM', 'MINOR_CIV_ARGOS', 'MINOR_CIV_ASSUR', 'MINOR_CIV_AZTLAN', 
'MINOR_CIV_BAN_CHIANG', 'MINOR_CIV_BRANDENBURG', 'MINOR_CIV_DJENNE', 'MINOR_CIV_DORESTAD', 'MINOR_CIV_ECBATANA', 'MINOR_CIV_ENNS', 
'MINOR_CIV_ERACLEA', 'MINOR_CIV_HARAPPA', 'MINOR_CIV_HEIDABIR', 'MINOR_CIV_HOLMUL', 'MINOR_CIV_HUARI', 'MINOR_CIV_KWA_BULAWAYO', 
'MINOR_CIV_LONGCHENG', 'MINOR_CIV_LUTETIA', 'MINOR_CIV_MSOURA', 'MINOR_CIV_NIPPUR', 'MINOR_CIV_NOVGOROD', 'MINOR_CIV_OKILIS', 
'MINOR_CIV_ONONDAGA', 'MINOR_CIV_OVIEDO', 'MINOR_CIV_PAGO_PAGO', 'MINOR_CIV_PALATIUM', 'MINOR_CIV_PERGE', 'MINOR_CIV_RAQMU', 'MINOR_CIV_SALEM', 
'MINOR_CIV_SHANGHAI', 'MINOR_CIV_SHEDET', 'MINOR_CIV_SIGTUNA', 'MINOR_CIV_SOGUT', 'MINOR_CIV_TROWULAN', 'MINOR_CIV_UTICA', 'MINOR_CIV_VELIGRAD', 
'MINOR_CIV_WANGGEOM', 'MINOR_CIV_WINCHESTER', 'MINOR_CIV_YAMATAI', 'MINOR_CIV_ZUUNMOD');
--==========================================================================================================================
-- ATLASES
--==========================================================================================================================	
...
SELECT 'TXT_KEY_CIV5_DAKKAR_TEXT', 							'Dakkar, also known as Dakar, or Deker, was a historical Muslim town located in present-day eastern Ethiopia. It served as the first capital of the Adal Sultanate (1415–1577) after its founding in the early 15th century by Sabr ad-Din III who reigned in the years 1415–1422. The Adal Sultanate was a powerful medieval Muslim state that flourished in the Horn of Africa, centered in what is now eastern Ethiopia, Djibouti, and northern Somalia. The sultanate played a key role in Red Sea and Indian Ocean trade, linking Africa with Arabia and Asia. It is most famous for its fierce conflict with the Ethiopian Empire during the early 16th century, led by the military commander Ahmad ibn Ibrahim al-Ghazi, also known as Ahmad "Gran". His campaigns nearly brought down the Ethiopian kingdom before his defeat and death in 1543. Though the sultanate eventually declined under internal and external pressures, it left a lasting legacy on the region’s political, cultural, and religious landscape.' UNION ALL
SELECT 'TXT_KEY_CITYSTATE_HONIARA', 					'Honiara' UNION ALL
SELECT 'TXT_KEY_CITYSTATE_HONIARA_ADJ', 					'Solomonian' UNION ALL
SELECT 'TXT_KEY_CIV5_HONIARA_TEXT', 						'Honiara is the capital and largest city of Solomon Islands, located on the northwestern coast of Guadalcanal. The area gained global attention during World War II as the site of intense fighting between Allied and Japanese forces, particularly during the Guadalcanal campaign (1942–1943). After the war, Honiara was chosen to replace Tulagi as the national capital because of its better infrastructure and strategic location. Officially established as the capital in 1952, the city developed rapidly, becoming the country''s administrative, economic, and cultural hub. Today, Honiara is home to government institutions, a busy port, and a diverse population, while remnants of the war, such as memorials and relics, remain visible, linking its modern identity to its pivotal wartime past.' UNION ALL
SELECT 'TXT_KEY_CITYSTATE_KARASJOHKA', 					'Karasjohka' UNION ALL
SELECT 'TXT_KEY_CITYSTATE_KARASJOHKA_ADJ', 					'Sami' UNION ALL
SELECT 'TXT_KEY_CIV5_KARASJOHKA_TEXT', 						'The Sami are an Indigenous people native to the northern regions of Norway, Sweden, Finland, and Russia''s Kola Peninsula, a cultural region known as Sápmi. With a history stretching back thousands of years, the Sami traditionally lived as semi-nomadic reindeer herders, fishers, and hunters, maintaining a close relationship with the Arctic environment. Their culture is rich in oral traditions, handicrafts (duodji), and distinctive clothing, such as the gákti. From the 19th century onward, they faced assimilation pressures and loss of land rights under national governments. In recent decades, however, there has been a strong cultural and political revival, with the establishment of Sami parliaments and increased recognition of their rights. Today, the Sami continue to balance modern life with the preservation of their language, traditions, and ancestral lands.' UNION ALL
...
SELECT 'TXT_KEY_CIV5_PALMYRA_TEXT', 						'Palmyra, also called Tadmur, Tadmor, or Tudmur, is an ancient city in south-central Syria, 210 km northeast of Damascus. The name Palmyra, meaning "city of palm trees," was conferred upon the city by its Roman rulers in the 1st century AD; Tadmur, Tadmor, or Tudmur, the pre-Semitic name of the site, is also still in use. The city is mentioned in tablets dating from as early as the 19th century BC. It attained prominence in the 3rd century bce, when a road through it became one of the main routes of east-west trade. Palmyra was built on an oasis lying approximately halfway between the Mediterranean Sea (west) and the Euphrates River (east), and it helped connect the Roman world with Mesopotamia and the East.' UNION ALL
SELECT 'TXT_KEY_CITYSTATE_AL_TIRABIN', 					'Al-Tirabin' UNION ALL
SELECT 'TXT_KEY_CITYSTATE_AL_TIRABIN_ADJ', 					'Bedouin' UNION ALL
SELECT 'TXT_KEY_CIV5_AL_TIRABIN_TEXT', 						'The Bedouins are traditionally nomadic Arab tribes known for their deep-rooted culture, pastoral lifestyle, and strong sense of tribal identity, originating from the deserts of the Arabian Peninsula and spreading across North Africa, the Levant, and parts of the Middle East. Historically, they lived by herding camels, goats, and sheep, moving seasonally in search of water and grazing lands, and became skilled in desert survival, oral poetry, and hospitality. Their social structure was built around kinship, honor, and loyalty to the tribe, with leaders known as sheikhs. Over time, as modern states expanded and urbanization increased in the 20th century, many Bedouins settled in towns or adopted semi-nomadic lives, though elements of their traditions, such as dress, poetry, and music—remain vital symbols of Arab cultural heritage and identity.' UNION ALL
SELECT 'TXT_KEY_CITYSTATE_HANUABADA', 					'Hanuabada' UNION ALL
SELECT 'TXT_KEY_CITYSTATE_HANUABADA_ADJ', 					'Motu' UNION ALL
SELECT 'TXT_KEY_CIV5_HANUABADA_TEXT', 						'The Motu are people from southeastern Papua New Guinea and were the original people of the land of Port Moresby, the capital and largest city in the country. The largest Motu settlement and traditional centre of the group is the town of Hanuabada, not too far up the coast from Port Moresby. Today it is almost part of Port Moresby itself as it is only a few kilometres northwest. The name Hanuabada comes from the Motu language and simply means "big village". Their language is an Austronesian language, but some today, speak an English based pidgin called Tok Pisin. The Motu lived on the stretch of coastline first settled permanently by Europeans on the island and are thus, more well-chronicled than others. They also have a tradition of sailing, using multihulled rafts with crab-claw shaped sails made from coconut fibre to trade with others near them. The Motu inhabited mostly along the bridge, not moving far inland as they were traders and fishermen.' UNION ALL
SELECT 'TXT_KEY_CITYSTATE_BALKH', 						'Bactra' UNION ALL
SELECT 'TXT_KEY_CITYSTATE_BALKH_ADJ', 						'Bactrian' UNION ALL
SELECT 'TXT_KEY_CIV5_BALKH_TEXT', 							'Bactra, known today as Balkh in northern Afghanistan, is one of the oldest cities in the world and was a major center of ancient civilization in Central Asia. Situated along the Silk Road, it served as the capital of the ancient region of Bactria and flourished as a hub of trade, culture, and religion. Believed to have been founded before 2000 BCE, Bactra was successively ruled by the Achaemenid Persians, conquered by Alexander the Great in the 4th century BCE, and later became the heart of the Greco-Bactrian Kingdom. It was also a key center of Zoroastrianism and, later, Buddhism, with numerous monasteries and stupas that influenced spiritual life across Asia. Despite repeated invasions from the Kushans to the Arabs and Mongols, Bactra retained its reputation as the "Mother of Cities", symbolizing the enduring legacy of ancient Iranian and Central Asian civilization.' UNION ALL
SELECT 'TXT_KEY_CITYSTATE_IRUNEA', 						'Donostia' UNION ALL
SELECT 'TXT_KEY_CITYSTATE_IRUNEA_ADJ', 						'Iruñean' UNION ALL
SELECT 'TXT_KEY_CIV5_IRUNEA_TEXT', 							'Donostia, also known as San Sebastián in Spanish, is a coastal city in the Basque Country of northern Spain, celebrated for its stunning crescent-shaped La Concha Bay, elegant architecture, and rich cultural life. Founded in the 12th century by King Sancho VI of Navarre as a strategic port and fortress, the city grew into an important maritime and commercial hub on the Bay of Biscay. After being largely destroyed during the Peninsular War in 1813, Donostia was rebuilt in the 19th century in a Belle Époque style, becoming a favored summer retreat for Spanish royalty and aristocracy. Today, it is renowned for its world-class gastronomy, international film and jazz festivals, and strong Basque identity, blending traditional culture with cosmopolitan charm to make it one of Europe’s most vibrant seaside cities.' UNION ALL
SELECT 'TXT_KEY_CITYSTATE_SADDARVAZEH', 				'Saddarvazeh' UNION ALL
SELECT 'TXT_KEY_CITYSTATE_SADDARVAZEH_ADJ', 				'Achaemenid' UNION ALL
SELECT 'TXT_KEY_CIV5_SADDARVAZEH_TEXT', 					'Saddarvazeh or Hecatompylos, meaning "City of a Hundred Gates" in Persian and Greek, respectively, was an ancient city located in northeastern Iran, near modern-day Damghan or Shahrud. It served as the capital of the Parthian Empire under the Arsacid dynasty around the 3rd century BCE, becoming a major political and commercial hub on the Silk Road connecting Mesopotamia with Central Asia. Originally a Seleucid foundation or an older Hellenistic settlement rebuilt by the Parthians, Hecatompylos thrived as a royal residence and strategic stronghold until it was later eclipsed by other Parthian capitals such as Ctesiphon. Although little remains of the city today, its ruins mostly identified at the site of Sahr-e Qumis, ancient writers like Strabo and Pliny the Elder praised its grandeur and its position as a gateway between East and West.' UNION ALL
SELECT 'TXT_KEY_CITYSTATE_JETARKTE', 					'Jetarkte' UNION ALL
SELECT 'TXT_KEY_CITYSTATE_JETARKTE_ADJ', 					'Kaweskar' UNION ALL
SELECT 'TXT_KEY_CIV5_JETARKTE_TEXT', 						'The history of the Kaweskar people is a story of a fruitless struggle of a people dearly trying to keep their traditions alive in an uncaring world that just seems to haphazardly lend them a hand at the best of times. A people who were witnessed at the very early days of American exploration yet weren''t fully contacted until well entered the 20th century. People living in one of the harshest environments the world could offer while seeing the world move around them, not relevant to establish permanent trade with, yet not possessing enough wealth to seek out to fight them either. A world ravaged by plague which decimated their population, directing them onto a path of annihilation that made them lose their very faith on the spirits of the Earth. Yet people who still managed to overcome, to survive, and to live to this day. The very earlier contacts with the Kaweskar were done during the days of Magellan and the early explorers of the new world, yet this territory wasn''t truly colonized by the white man until the 20th century, a time when their population was already decimated due to the overhunting of their traditional food sources and the illnesses they had no immunity to that had made their presence on this land.' UNION ALL
SELECT 'TXT_KEY_CITYSTATE_MBABANE', 					'Mbabane' UNION ALL
SELECT 'TXT_KEY_CITYSTATE_MBABANE_ADJ', 					'Eswatinian' UNION ALL
SELECT 'TXT_KEY_CIV5_MBABANE_TEXT', 						'Mbabane, the capital and largest city of Eswatini (formerly Swaziland), is located in the country''s northwestern highlands along the Mbabane River, surrounded by the scenic Dlangeni and Mdzimba Mountains. Established as a colonial administrative center by the British in the early 1900s, it became the capital of the British protectorate of Swaziland in 1902 and later retained its status after the nation gained independence in 1968. Named after Chief Mbabane Kunene, the area''s early inhabitant, the city grew around its government institutions and trade routes, developing into the political and commercial heart of the country. Today, Mbabane blends modern urban life with traditional Swazi culture and serves as a gateway to Eswatini''s mountain landscapes, making it both an administrative hub and a center of cultural and economic activity.' UNION ALL
SELECT 'TXT_KEY_CITYSTATE_TUNIS', 						'Tunis' UNION ALL
SELECT 'TXT_KEY_CITYSTATE_TUNIS_ADJ', 						'Tunisian' UNION ALL
SELECT 'TXT_KEY_CIV5_TUNIS_TEXT', 							'Tunis, the capital and largest city of Tunisia, lies on the Mediterranean coast near the ancient ruins of Carthage and serves as the political, economic, and cultural heart of the country. Its history stretches back to at least the 9th century BCE, when it began as a Berber settlement later influenced by Phoenician Carthage, Roman rule, and Islamic conquest in the 7th century CE. Under the Aghlabids and later the Hafsids, Tunis rose to prominence as a major center of learning, trade, and Islamic culture in North Africa. The city flourished further under Ottoman and then French colonial rule, blending Arab, Mediterranean, and European influences in its architecture and society. Following Tunisia''s independence in 1956, Tunis became the seat of government and a symbol of national identity. Today, its bustling medina, recognized as a UNESCO World Heritage Site, stands as a living testament to over two millennia of history and cultural exchange.' UNION ALL
SELECT 'TXT_KEY_CITYSTATE_MENDYARRUP', 					'Mendyarrup' UNION ALL
SELECT 'TXT_KEY_CITYSTATE_MENDYARRUP_ADJ', 					'Noongar' UNION ALL
SELECT 'TXT_KEY_CIV5_MENDYARRUP_TEXT', 						'The Noongar are the Aboriginal people of the southwest region of Western Australia, whose ancestral lands stretch from Jurien Bay in the north to Esperance in the southeast, encompassing the area around Perth. With a history spanning over 45,000 years, the Noongar have maintained a deep spiritual and cultural connection to boodja (land), guided by the Dreaming stories that explain the creation of their environment and laws. Traditionally, Noongar society consisted of interconnected groups with their own territories, languages, and customs, relying on seasonal movement, hunting, and gathering for sustenance. Following British colonization in 1829, the Noongar faced displacement, loss of land, and suppression of culture, yet they have continued to preserve and revive their language, traditions, and identity. Today, the Noongar people play a vital role in cultural education, environmental stewardship, and reconciliation efforts across Western Australia, embodying one of the oldest continuous living cultures on Earth.' UNION ALL
SELECT 'TXT_KEY_CITYSTATE_BALLEY_CHASHTAL', 			'Balley y Chashtal' UNION ALL
SELECT 'TXT_KEY_CITYSTATE_BALLEY_CHASHTAL_ADJ', 			'Norse-Gaelic' UNION ALL
SELECT 'TXT_KEY_CIV5_BALLEY_CHASHTAL_TEXT', 				'Balley y Chashtal, also known as the "Town of Castletown," is one of the most historically significant settlements on the Isle of Man. Located at the island''s southern tip, it developed around Castle Rushen, a medieval fortress built in the late 12th century. Serving as the island''s capital until 1869, Balley y Chashtal was the administrative and political center of the Isle of Man for centuries. Its strategic coastal position made it a hub for trade and defense. Today, it retains its rich heritage through well-preserved Georgian and Victorian architecture, cobbled streets, and landmarks like Old House of Keys, once home to the island''s parliament. The town remains a symbol of Manx identity and history, blending its medieval past with a vibrant community.' UNION ALL
SELECT 'TXT_KEY_CITYSTATE_KAGYRGYN', 					'Kagyrgyn' UNION ALL
SELECT 'TXT_KEY_CITYSTATE_KAGYRGYN_ADJ', 					'Chukchi' UNION ALL
SELECT 'TXT_KEY_CIV5_KAGYRGYN_TEXT', 						'The Chukchi are an Indigenous people native to the Chukotka Peninsula in the far northeastern part of Siberia, Russia. Traditionally, they are divided into two main groups: the "maritime Chukchi", who lived along the Arctic coast and relied on hunting whales, walruses, and seals, and the "reindeer Chukchi", who herded reindeer inland. Their culture is deeply rooted in animism, shamanism, and a close relationship with the natural environment. First recorded contact with Russians occurred in the 17th century, leading to conflicts and eventual integration under the Russian Empire. Despite Soviet-era pressures to assimilate, the Chukchi preserved much of their language, traditions, and nomadic way of life. Today, many still live in their ancestral lands, maintaining cultural practices while also participating in modern Russian society.' UNION ALL
SELECT 'TXT_KEY_CITYSTATE_VADUZ', 						'Vaduz' UNION ALL
SELECT 'TXT_KEY_CITYSTATE_VADUZ_ADJ', 						'Liechtensteiner' UNION ALL
SELECT 'TXT_KEY_CIV5_VADUZ_TEXT', 							'Vaduz is the capital of Liechtenstein, nestled along the Rhine River at the foot of the Alps. Though small in size, it holds great historical and political significance. The area was first settled in the medieval period, and Vaduz was officially founded in the 14th century. It became the administrative center after the purchase of the county by the Liechtenstein family in 1712, eventually giving its name to the entire principality. Over time, Vaduz grew from a modest village into a political and cultural hub, particularly after Liechtenstein gained full sovereignty in 1866. Dominating the town is Vaduz Castle, the residence of the princely family. Today, Vaduz is known for its blend of historic charm and modern institutions, including museums, government buildings, and financial centers, while remaining one of Europe''s most picturesque capitals.' UNION ALL
SELECT 'TXT_KEY_CITYSTATE_MATONJENI', 					'Matonjeni' UNION ALL
SELECT 'TXT_KEY_CITYSTATE_MATONJENI_ADJ', 					'Kalanga' UNION ALL
SELECT 'TXT_KEY_CIV5_MATONJENI_TEXT', 						'The Njelele Shrine or Matonjeni is an emblematic sacred site in Zimbabwe and bears profound cultural and spiritual significance rooted in the traditions of rainmaking rituals and ancestral veneration. Revered by local communities for its role in facilitating communication with ancestral spirits and petitioning for rain and agricultural abundance, the shrine embodies centuries-old beliefs and practices passed down through generations. Matonjeni is also related to the cult of Mwari (Mwali), the supreme creator deity according to Shona and Venda traditional religion practiced in the Kingdom of Zimbabwe and in its successors Mutapa and Butua. Pilgrimages and ceremonies conducted at Njelele Shrine serve as vital communal and spiritual rites, preserving the cultural heritage and traditions of Zimbabwe''s indigenous peoples while fostering a deep connection to their ancestral past and the natural world.' UNION ALL
SELECT 'TXT_KEY_CITYSTATE_NGAZARGAMU', 					'Ngazargamu' UNION ALL
SELECT 'TXT_KEY_CITYSTATE_NGAZARGAMU_ADJ', 					'Kanem-Bornu' UNION ALL
SELECT 'TXT_KEY_CIV5_NGAZARGAMU_TEXT', 						'The Kanem-Bornu Empire was an empire that existed in what is now modern day Chad and Nigeria. It was known to Arab geographers as the Kanem Empire from the 9th century AD onwards and lasted as the independent kingdom of Bornu until 1900. The history of the empire is mainly known from the Royal Chronicle or Girgam discovered in 1851 by the German traveler Heinrich Barth. At its height, the Kanem-Bornu Empire encompassed an area covering not only much of Chad, but also parts of modern southern Libya, eastern Niger, northeastern Nigeria and northern Cameroon. It was located at the southern end of the trans-Saharan trade route between Tripoli and the region of Lake Chad. It is a hot, arid region with several oases, the largest being Lake Chad.' UNION ALL
SELECT 'TXT_KEY_CITYSTATE_SANANDAJ', 					'Sanandaj' UNION ALL
SELECT 'TXT_KEY_CITYSTATE_SANANDAJ_ADJ', 					'Gorani' UNION ALL
SELECT 'TXT_KEY_CIV5_SANANDAJ_TEXT', 						'Sanandaj, the capital of Iran''s Kurdistan Province, lies in the Zagros Mountains in western Iran and is renowned as a cultural and historical center of Kurdish life. The city''s name, derived from the Persian Sena Dezh meaning "Castle of Sena", reflects its ancient roots, which trace back to at least the 17th century when it developed significantly under Safavid rule. Sanandaj flourished as an administrative and commercial hub during the Qajar period, known for its distinctive Kurdish architecture, traditional crafts such as carpet weaving, and vibrant music. Despite modern growth, the city retains much of its cultural identity through its bazaars, mosques, and historical mansions like Khosro Abad Palace. Today, Sanandaj stands as both a symbol of Kurdish cultural pride and a bridge between Iran''s mountainous Kurdish regions and the wider Iranian plateau.' UNION ALL
SELECT 'TXT_KEY_CITYSTATE_BAGHCASARAY', 				'Baghcasaray' UNION ALL
SELECT 'TXT_KEY_CITYSTATE_BAGHCASARAY_ADJ', 				'Crimean' UNION ALL
SELECT 'TXT_KEY_CIV5_BAGHCASARAY_TEXT', 					'Bakhchisaray (also spelled Baghchasaray or Bakhchysarai) is a historic town in Crimea, Ukraine, known as the former capital of the Crimean Khanate from the 16th to the 18th century. Founded around 1532 by Sahib I Giray, it became the political, cultural, and spiritual center of the Crimean Tatars. The town''s name, derived from Persian (baghche-saray meaning "garden palace"), reflects its picturesque setting among gardens and mountain valleys. Its most famous landmark, the Bakhchisaray Palace (Hansaray), served as the residence of the Crimean khans and is celebrated for its Islamic architecture, intricate tilework, and the "Fountain of Tears", immortalized by Alexander Pushkin in his 1823 poem. Though the city declined after the Russian annexation of Crimea in 1783, it remains a symbol of Crimean Tatar heritage and resilience.' UNION ALL

SELECT 'TXT_KEY_CITYSTATE_ISHIYAMA', 					'Ishiyama' UNION ALL
SELECT 'TXT_KEY_CITYSTATE_ISHIYAMA_ADJ', 					'Ikko-ikki' UNION ALL
SELECT 'TXT_KEY_CIV5_ISHIYAMA_TEXT', 						'The Ikko-ikki were militant groups of Buddhist monks, peasants, and local warriors in 15th–16th century Japan who rose up against feudal lords and samurai rule. Centered around the teachings of the Jodo Shinshu (True Pure Land) sect of Buddhism, they were united by religious devotion and a desire for social equality. Emerging during the chaotic Sengoku period, these leagues established autonomous communities in several provinces, most notably controlling Ishiyama Hongan-ji fortress temple near Osaka. Their strong resistance challenged powerful warlords, including Oda Nobunaga, who eventually crushed the movement in the late 16th century. Though defeated, the Ikko-ikki left a lasting legacy as one of the earliest large-scale, grassroots movements in Japanese history, blending faith with political resistance.';
...
SELECT 'TXT_KEY_CSTRAIT_MINOR_CIV_BAGHCASARAY', 		'[COLOR_MAGENTA]Desht-i-Kipchak Successors:[ENDCOLOR] [COLOR_YIELD_GOLD]Mounted and Armored Units[ENDCOLOR] gain [COLOR_WATER_TEXT]Yesir Captives[ENDCOLOR] promotion (Unit has 5% chance to add 1 [ICON_CITIZEN] Citizen to the nearest City [COLOR:255:230:85:255]on Kill[ENDCOLOR]).[NEWLINE]' UNION ALL
...
SELECT 'TXT_KEY_CSTRAIT_MINOR_CIV_BALLEY_CHASHTAL', 	'[COLOR_MAGENTA]Norse-Gaelic Unity:[ENDCOLOR] [ICON_GREAT_WORK] Great Works gain +1 [ICON_FOOD] Food and 1 [ICON_CULTURE] Culture. Cities gain +10 [ICON_FOOD] Food for each building with themed [ICON_GREAT_WORK] Great Works.[NEWLINE]' UNION ALL
...
SELECT 'TXT_KEY_CSTRAIT_MINOR_CIV_HONIARA', 			'[COLOR_MAGENTA]Suahongi Dance:[ENDCOLOR] [ICON_GW_MUSIC] Great Works of Music gain +2 [ICON_FOOD] Food. Musicians gain +1 [ICON_CULTURE_LOCAL] Border Growth Point. Each [ICON_CULTURE] Culture Expansion to a [COLOR_GREY]Bonus Resource[ENDCOLOR] grants 25 [ICON_FOOD] Food on land and 50 [ICON_FOOD] Food on sea, scaling with Era.[NEWLINE]' UNION ALL
...
SELECT 'TXT_KEY_CSTRAIT_MINOR_CIV_KAGYRGYN', 			'[COLOR_MAGENTA]Luoravetlan:[ENDCOLOR] [COLOR_CITY_BROWN]Camps[ENDCOLOR] and [COLOR_CITY_BROWN]Fishing Boats[ENDCOLOR] gain +1 [ICON_GREAT_GENERAL] Great General Point. Birth of a [ICON_GREAT_GENERAL] [COLOR_YIELD_GOLD]Great General[ENDCOLOR] grants in the origin City [ICON_FOOD] Food equal to 75% of current [ICON_FOOD] Food per turn value of your Empire.[NEWLINE]' UNION ALL
...
SELECT 'TXT_KEY_CSTRAIT_MINOR_CIV_MATONJENI', 			'[COLOR_MAGENTA]Sacred Abode of Mwali:[ENDCOLOR] [COLOR_CITY_BROWN]Holy Sites[ENDCOLOR] gain +4 [ICON_FOOD] Food. Birth of a [ICON_PROPHET] [COLOR_YIELD_GOLD]Great Prophet[ENDCOLOR] grants in the origin City [ICON_FOOD] Food equal to 125% of current [ICON_FOOD] Food per turn value of your Empire.[NEWLINE]' UNION ALL
...
SELECT 'TXT_KEY_CSTRAIT_MINOR_CIV_MOGADISHU', 			'[COLOR_MAGENTA]Maritime Growth:[ENDCOLOR] +10% [ICON_FOOD] Food during the [ICON_GOLDEN_AGE] Golden Ages. -5% [ICON_GOLDEN_AGE] Golden Age Cost. Player can build a [COLOR_CITY_BROWN]Daaq Ah[ENDCOLOR] improvement ([ICON_FOOD], [ICON_PRODUCTION], [ICON_GOLD], [ICON_CULTURE], [ICON_CULTURE_LOCAL]) on Hills with [COLOR_CITY_BLUE]Plains[ENDCOLOR] or [COLOR_CITY_BLUE]Desert[ENDCOLOR] or on Hills with [COLOR_GREY]Bonus Pasture Resource[ENDCOLOR].[NEWLINE]' UNION ALL
...
SELECT 'TXT_KEY_CSTRAIT_MINOR_CIV_NGAZARGAMU', 			'[COLOR_MAGENTA]None to Fear but God:[ENDCOLOR] [COLOR_CITY_BROWN]Towns[ENDCOLOR] and [COLOR_CITY_BROWN]Villages[ENDCOLOR] gain +1 [ICON_PRODUCTION] Production. Training a Military Unit grants 2 [ICON_PEACE] Faith in all Cities, scaling with Era.[NEWLINE]' UNION ALL
...
SELECT 'TXT_KEY_CSTRAIT_MINOR_CIV_SANAA', 				'[COLOR_MAGENTA]Arabia Felix:[ENDCOLOR] Capital gains +1 [ICON_GOLD] Gold for each different [COLOR_GREY]Luxury Resource[ENDCOLOR] improved by the Player (traded or imported [COLOR_GREY]Luxury Resources[ENDCOLOR] do not count). Each currently applied percentage-based [ICON_MONOPOLY] Global Monopoly modifier gains an additional +5% bonus.[NEWLINE]' UNION ALL
SELECT 'TXT_KEY_CSTRAIT_MINOR_CIV_SANANDAJ', 			'[COLOR_MAGENTA]Gorani Literature:[ENDCOLOR] Training a Diplomatic Unit grants 15-25 [ICON_CULTURE] Culture in the City, scaling with Era. Each Era change grants a free [COLOR_YIELD_GOLD]Envoy[ENDCOLOR] in your Capital.[NEWLINE]' UNION ALL
SELECT 'TXT_KEY_CSTRAIT_MINOR_CIV_SANTO_DOMINGO', 		'[COLOR_MAGENTA]Caribbean''s Gateway:[ENDCOLOR] Each currently applied Happiness or flat-yield [ICON_MONOPOLY] Global Monopoly modifier gains an additional +2 bonus.[NEWLINE]' UNION ALL
...
SELECT 'TXT_KEY_CSTRAIT_MINOR_CIV_TYRE', 				'[COLOR_MAGENTA]Ruins of the Ancient World:[ENDCOLOR] [ICON_ARTIFACT] [COLOR_CITY_BROWN]Landmarks[ENDCOLOR] gain +2 [ICON_GOLD] Gold. Cities gain +10% Production towards [ICON_ARTIFACT] [COLOR_YIELD_GOLD]Archaeologists[ENDCOLOR]. [ICON_WONDER] World Wonder construction grants 20-40 [ICON_TOURISM] Tourism, scaling with Era and number of [ICON_WONDER] World Wonders in that City.[NEWLINE]' UNION ALL
...
SELECT 'TXT_KEY_CSTRAIT_MINOR_CIV_VADUZ', 				'[COLOR_MAGENTA]Princely Collections:[ENDCOLOR] [ICON_GREAT_WORK] Great Works gain +1 [ICON_GOLD] Gold. [ICON_GOLD] Gold Purchase on a building grants 20-30 [ICON_GREAT_PEOPLE] Great People Points towards a random Cultural [ICON_GREAT_PEOPLE] Great Person in this City, scaling with Era. This value is reduced by 80%, if City currently has a progress towards that [ICON_GREAT_PEOPLE] Great Person.[NEWLINE]' UNION ALL
...
SELECT 'TXT_KEY_CSTRAIT_MINOR_CIV_WELLINGTON', 			'[COLOR_MAGENTA]Te Whanga-nui-a-Tara:[ENDCOLOR] [COLOR_GREY]Strategic Resources[ENDCOLOR] gain +1 [ICON_FOOD] Food. Player gains additional copies of [COLOR_GREY]Strategic Resource[ENDCOLOR]:[NEWLINE][ICON_BULLET] +1, if he has at least 1 copy and no Monopolies[NEWLINE][ICON_BULLET] +2, if he has a [ICON_MONOPOLY] Strategic Monopoly[NEWLINE][ICON_BULLET] +3, if he has a [ICON_MONOPOLY] Global Monopoly[NEWLINE]' UNION ALL
...
SELECT 'TXT_KEY_PROMOTION_BAGHCASARAY', 				'Yesir Captives' UNION ALL
SELECT 'TXT_KEY_PROMOTION_BAGHCASARAY_HELP', 				'On kill, unit has 10% chance to add 1 [ICON_CITIZEN] Citizen to the nearest City.' UNION ALL
...
SELECT 'TXT_KEY_CULTURED_FRIENDLY_BONUS', 					'[COLOR:240:180:235:255]Open University (Cultured Affable):[ENDCOLOR][NEWLINE][ICON_BULLET]1% Chance for spawning an [ICON_ARTIFACT] [COLOR_YIELD_GOLD]Archaeologist[ENDCOLOR] inside Friends'' Territory (after researching Archaeology)'  UNION ALL
...
SELECT 'TXT_KEY_BUILDING_ARMAGH1_HELP', 				'Requires an access to [COLOR_CYAN]Armagh[ENDCOLOR]''s ability.' UNION ALL
...
SELECT 'TXT_KEY_BUILDING_ARMAGH2_HELP', 				'Requires an access to [COLOR_CYAN]Armagh[ENDCOLOR]''s ability.' UNION ALL
...
SELECT 'TXT_KEY_BUILDING_ARMAGH3_HELP', 				'Requires an access to [COLOR_CYAN]Armagh[ENDCOLOR]''s ability.' UNION ALL
...
SELECT 'TXT_KEY_BUILDING_ARMAGH4_HELP', 				'Requires an access to [COLOR_CYAN]Armagh[ENDCOLOR]''s ability.' UNION ALL
...
SELECT 'TXT_KEY_BUILDING_JERUSALEM_HELP', 				'Built after adopting Majority Religion of its Ally.[NEWLINE][NEWLINE]This city becomes a Holy City. Too many have made the pilgrimage to this city for their religion.' UNION ALL
...
SELECT 'TXT_KEY_BUILDING_KIEV_HELP', 					'Requires an access to [COLOR_CYAN]Kyiv[ENDCOLOR]''s ability.' UNION ALL
...
SELECT 'TXT_KEY_BUILDING_MILAN_HELP', 					'Requires an access to [COLOR_CYAN]Milan[ENDCOLOR]''s ability.' UNION ALL
...
SELECT 'TXT_KEY_BUILDING_VILNIUS_HELP', 				'Requires an access to [COLOR_CYAN]Vilnius[ENDCOLOR]''s ability.' UNION ALL
...
SELECT 'TXT_KEY_BUILDING_RISHIKESH_HELP', 				'Requires an access to [COLOR_CYAN]Rishikesh[ENDCOLOR]''s ability.' UNION ALL
...
SELECT 'TXT_KEY_BUILDING_VALLETTA_HELP', 				'City must be on Coast and Player must have an access to [COLOR_CYAN]Valletta[ENDCOLOR]''s ability.[NEWLINE][NEWLINE]Yields [COLOR_NEGATIVE_TEXT]lost[ENDCOLOR] without the Alliance.' UNION ALL
-- world wonders
SELECT 'TXT_KEY_BUILDING_LHASA', 						'Potala Palace' UNION ALL
SELECT 'TXT_KEY_BUILDING_LHASA_QUOTE', 					'[NEWLINE]"The first time I stepped onto the rooftop of the Potala Palace, I felt, as never before or since, as if I were stepping onto the rooftop of my being; onto some dimension of consciousness that I''d never visited before."[NEWLINE] - Pico Iyer[NEWLINE]' UNION ALL
SELECT 'TXT_KEY_BUILDING_LHASA_PEDIA', 					'The Potala Palace, located in Lhasa, Tibet, is one of the most renowned architectural and cultural monuments in Asia. Its origins trace back to the 7th century when King Songtsen Gampo first built a fortress on the site, though the grand palace seen today was largely constructed in the 17th century under the leadership of the 5th Dalai Lama. Built atop Red Mountain at an elevation of 3,700 meters (12,100 feet), the palace became the center of Tibetan political and spiritual life for centuries. It served as the winter residence of successive Dalai Lamas and the seat of Tibetan governance until the mid-20th century, when Chinese administration was established in Tibet. Recognized as a UNESCO World Heritage Site in 1994, the palace embodies both the historical authority of Tibetan Buddhism and the region''s complex political history.[NEWLINE][NEWLINE]Architecturally, the Potala Palace is an extraordinary feat, blending fortress-like strength with ornate religious design. Rising 13 stories high and containing over 1,000 rooms, chapels, and shrines, the palace is divided into two main sections: the White Palace, which housed administrative offices and living quarters, and the Red Palace, dedicated to religious study, prayer, and elaborate tombs of Dalai Lamas. Its walls are adorned with murals depicting Buddhist cosmology, history, and teachings, while its treasury holds countless artifacts, scriptures, and statues of immense cultural value. Beyond its grandeur, the palace remains a symbol of Tibetan identity and spirituality, drawing pilgrims, historians, and tourists alike to its commanding silhouette over the Lhasa Valley.' UNION ALL
SELECT 'TXT_KEY_BUILDING_LHASA_HELP', 					'Requires an access to [COLOR_CYAN]Lhasa[ENDCOLOR]''s ability and an active session of [COLOR_POSITIVE_TEXT]World Congress[ENDCOLOR].[NEWLINE][NEWLINE]Triggers a [COLOR_YIELD_GOLD]Sphere of Influence[ENDCOLOR] over [COLOR_CYAN]Lhasa[ENDCOLOR].[NEWLINE]Yield modifiers [COLOR_NEGATIVE_TEXT]halved[ENDCOLOR] without the Alliance.';
---------------------------------------------------
---------------------------------------------------
--==========================================================================================================================
-- DEFINITIONS
...
INSERT INTO MinorCivilizations 
			(Type, 							ShortDescription, 						Civilopedia, 							Description, 							Adjective, 									ArtDefineTag, 					DefaultPlayerColor, 				ArtStyleType, 				ArtStyleSuffix, 	ArtStylePrefix, 	MinorCivTrait,					FixedPersonality,	Playable,	BullyUnitClass)
SELECT 		'MINOR_CIV_ADEJE', 				'TXT_KEY_CITYSTATE_ADEJE', 				'TXT_KEY_CIV5_ADEJE_TEXT', 				'TXT_KEY_CITYSTATE_ADEJE', 				'TXT_KEY_CITYSTATE_ADEJE_ADJ', 				'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_WHITE', 			'ARTSTYLE_MIDDLE_EAST', 	'_AFRI', 			'AFRICAN', 			'MINOR_TRAIT_MARITIME',			NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_AL_TIRABIN', 		'TXT_KEY_CITYSTATE_AL_TIRABIN', 		'TXT_KEY_CIV5_AL_TIRABIN_TEXT', 		'TXT_KEY_CITYSTATE_AL_TIRABIN', 		'TXT_KEY_CITYSTATE_AL_TIRABIN_ADJ', 		'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_GREEN', 			'ARTSTYLE_MIDDLE_EAST', 	'_ASIA', 			'ASIAN', 			'MINOR_TRAIT_MERCANTILE',		NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_AMBRACIA', 			'TXT_KEY_CITYSTATE_AMBRACIA', 			'TXT_KEY_CIV5_AMBRACIA_TEXT', 			'TXT_KEY_CITYSTATE_AMBRACIA', 			'TXT_KEY_CITYSTATE_AMBRACIA_ADJ', 			'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_LIGHT_GREEN', 	'ARTSTYLE_GRECO_ROMAN', 	'_MED', 			'MEDITERRANEAN', 	'MINOR_TRAIT_MILITARISTIC',		NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_ANANGULA', 			'TXT_KEY_CITYSTATE_ANANGULA', 			'TXT_KEY_CIV5_ANANGULA_TEXT',			'TXT_KEY_CITYSTATE_ANANGULA',			'TXT_KEY_CITYSTATE_ANANGULA_ADJ', 			'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_WHITE', 			'ARTSTYLE_SOUTH_AMERICA', 	'_AMER', 			'AMERICAN', 		'MINOR_TRAIT_CULTURED',			NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_ANDORRA', 			'TXT_KEY_CITYSTATE_ANDORRA', 			'TXT_KEY_CIV5_ANDORRA_TEXT', 			'TXT_KEY_CITYSTATE_ANDORRA', 			'TXT_KEY_CITYSTATE_ANDORRA_ADJ', 			'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_BLUE', 			'ARTSTYLE_GRECO_ROMAN', 	'_MED', 			'MEDITERRANEAN', 	'MINOR_TRAIT_CULTURED',			NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_ARMAGH', 			'TXT_KEY_CITYSTATE_ARMAGH', 			'TXT_KEY_CIV5_ARMAGH_TEXT', 			'TXT_KEY_CITYSTATE_ARMAGH', 			'TXT_KEY_CITYSTATE_ARMAGH_ADJ', 			'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_CYAN', 			'ARTSTYLE_EUROPEAN', 		'_EURO', 			'EUROPEAN', 		'MINOR_TRAIT_RELIGIOUS',		NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_AUCKLAND', 			'TXT_KEY_CITYSTATE_AUCKLAND', 			'TXT_KEY_CIV5_AUCKLAND_TEXT', 			'TXT_KEY_CITYSTATE_AUCKLAND', 			'TXT_KEY_CITYSTATE_AUCKLAND_ADJ', 			'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_BLUE', 			'ARTSTYLE_POLYNESIAN', 		'_POLYNESIA', 		'ASIAN', 			'MINOR_TRAIT_MARITIME',			NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_AYUTTHAYA', 			'TXT_KEY_CITYSTATE_AYUTTHAYA', 			'TXT_KEY_CIV5_AYUTTHAYA_TEXT', 			'TXT_KEY_CITYSTATE_AYUTTHAYA', 			'TXT_KEY_CITYSTATE_AYUTTHAYA_ADJ', 			'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_LIGHT_GREEN', 	'ARTSTYLE_MIDDLE_EAST', 	'_ASIA', 			'ASIAN', 			'MINOR_TRAIT_CULTURED',			NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_BAGAN', 				'TXT_KEY_CITYSTATE_BAGAN', 				'TXT_KEY_CIV5_BAGAN_TEXT', 				'TXT_KEY_CITYSTATE_BAGAN', 				'TXT_KEY_CITYSTATE_BAGAN_ADJ', 				'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_LIGHT_GREEN', 	'ARTSTYLE_ASIAN', 			'_ASIA', 			'ASIAN', 			'MINOR_TRAIT_RELIGIOUS',		NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_BAGHCASARAY', 		'TXT_KEY_CITYSTATE_BAGHCASARAY', 		'TXT_KEY_CIV5_BAGHCASARAY_TEXT', 		'TXT_KEY_CITYSTATE_BAGHCASARAY', 		'TXT_KEY_CITYSTATE_BAGHCASARAY_ADJ', 		'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_LIGHT_GREEN', 	'ARTSTYLE_EUROPEAN', 		'_ASIA', 			'ASIAN', 			'MINOR_TRAIT_MILITARISTIC',		NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_BALKH', 				'TXT_KEY_CITYSTATE_BALKH', 				'TXT_KEY_CIV5_BALKH_TEXT', 				'TXT_KEY_CITYSTATE_BALKH', 				'TXT_KEY_CITYSTATE_BALKH_ADJ', 				'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_CYAN', 			'ARTSTYLE_ASIAN', 			'_ASIA', 			'ASIAN', 			'MINOR_TRAIT_MILITARISTIC',		NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_BALLEY_CHASHTAL', 	'TXT_KEY_CITYSTATE_BALLEY_CHASHTAL', 	'TXT_KEY_CIV5_BALLEY_CHASHTAL_TEXT', 	'TXT_KEY_CITYSTATE_BALLEY_CHASHTAL', 	'TXT_KEY_CITYSTATE_BALLEY_CHASHTAL_ADJ', 	'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_CYAN', 			'ARTSTYLE_EUROPEAN', 		'_EURO', 			'EUROPEAN', 		'MINOR_TRAIT_MARITIME',			NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_BEDULU', 			'TXT_KEY_CITYSTATE_BEDULU', 			'TXT_KEY_CIV5_BEDULU_TEXT', 			'TXT_KEY_CITYSTATE_BEDULU', 			'TXT_KEY_CITYSTATE_BEDULU_ADJ', 			'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_CYAN', 			'ARTSTYLE_MIDDLE_EAST', 	'_ASIA', 			'ASIAN', 			'MINOR_TRAIT_RELIGIOUS',		NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_CANOSSA', 			'TXT_KEY_CITYSTATE_CANOSSA', 			'TXT_KEY_CIV5_CANOSSA_TEXT', 			'TXT_KEY_CITYSTATE_CANOSSA', 			'TXT_KEY_CITYSTATE_CANOSSA_ADJ', 			'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_GREEN', 			'ARTSTYLE_GRECO_ROMAN', 	'_MED', 			'MEDITERRANEAN', 	'MINOR_TRAIT_RELIGIOUS',		NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_CHEVAK', 			'TXT_KEY_CITYSTATE_CHEVAK', 			'TXT_KEY_CIV5_CHEVAK_TEXT', 			'TXT_KEY_CITYSTATE_CHEVAK', 			'TXT_KEY_CITYSTATE_CHEVAK_ADJ', 			'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_CYAN', 			'ARTSTYLE_SOUTH_AMERICA', 	'_AMER', 			'AMERICAN', 		'MINOR_TRAIT_MARITIME',			NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_CLERMONT', 			'TXT_KEY_CITYSTATE_CLERMONT', 			'TXT_KEY_CIV5_CLERMONT_TEXT', 			'TXT_KEY_CITYSTATE_CLERMONT', 			'TXT_KEY_CITYSTATE_CLERMONT_ADJ', 			'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_WHITE', 			'ARTSTYLE_EUROPEAN', 		'_EURO', 			'EUROPEAN', 		'MINOR_TRAIT_MILITARISTIC',		NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_DAKKAR', 			'TXT_KEY_CITYSTATE_DAKKAR', 			'TXT_KEY_CIV5_DAKKAR_TEXT', 			'TXT_KEY_CITYSTATE_DAKKAR', 			'TXT_KEY_CITYSTATE_DAKKAR_ADJ', 			'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_LIGHT_GREEN', 	'ARTSTYLE_MIDDLE_EAST', 	'_AFRI', 			'AFRICAN', 			'MINOR_TRAIT_MILITARISTIC',		NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_DALI', 				'TXT_KEY_CITYSTATE_DALI', 				'TXT_KEY_CIV5_DALI_TEXT', 				'TXT_KEY_CITYSTATE_DALI', 				'TXT_KEY_CITYSTATE_DALI_ADJ', 				'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_PEACH', 			'ARTSTYLE_ASIAN', 			'_ASIA', 			'ASIAN', 			'MINOR_TRAIT_RELIGIOUS',		NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_DANO', 				'TXT_KEY_CITYSTATE_DANO', 				'TXT_KEY_CIV5_DANO_TEXT', 				'TXT_KEY_CITYSTATE_DANO', 				'TXT_KEY_CITYSTATE_DANO_ADJ', 				'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_MIDDLE_BLUE', 	'ARTSTYLE_MIDDLE_EAST', 	'_AFRI', 			'AFRICAN', 			'MINOR_TRAIT_RELIGIOUS',		NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_DJIBOUTI', 			'TXT_KEY_CITYSTATE_DJIBOUTI', 			'TXT_KEY_CIV5_DJIBOUTI_TEXT', 			'TXT_KEY_CITYSTATE_DJIBOUTI', 			'TXT_KEY_CITYSTATE_DJIBOUTI_ADJ', 			'ART_DEF_CIVILIZATION_MINOR',	'PLAYERCOLOR_MINOR_PEACH', 			'ARTSTYLE_MIDDLE_EAST', 	'_AFRI', 			'AFRICAN', 			'MINOR_TRAIT_MARITIME',			NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_DODOMA', 			'TXT_KEY_CITYSTATE_DODOMA', 			'TXT_KEY_CIV5_DODOMA_TEXT', 			'TXT_KEY_CITYSTATE_DODOMA', 			'TXT_KEY_CITYSTATE_DODOMA_ADJ', 			'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_YELLOW', 		'ARTSTYLE_MIDDLE_EAST', 	'_AFRI', 			'AFRICAN', 			'MINOR_TRAIT_MERCANTILE',		NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_DOUALA', 			'TXT_KEY_CITYSTATE_DOUALA', 			'TXT_KEY_CIV5_DOUALA_TEXT', 			'TXT_KEY_CITYSTATE_DOUALA', 			'TXT_KEY_CITYSTATE_DOUALA_ADJ', 			'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_MIDDLE_BLUE', 	'ARTSTYLE_MIDDLE_EAST', 	'_AFRI', 			'AFRICAN', 			'MINOR_TRAIT_MERCANTILE',		NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_FAYA', 				'TXT_KEY_CITYSTATE_FAYA', 				'TXT_KEY_CIV5_FAYA_TEXT', 				'TXT_KEY_CITYSTATE_FAYA', 				'TXT_KEY_CITYSTATE_FAYA_ADJ', 				'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_GREEN', 			'ARTSTYLE_MIDDLE_EAST', 	'_AFRI', 			'AFRICAN', 			'MINOR_TRAIT_MILITARISTIC',		NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_GRANADA', 			'TXT_KEY_CITYSTATE_GRANADA', 			'TXT_KEY_CIV5_GRANADA_TEXT', 			'TXT_KEY_CITYSTATE_GRANADA', 			'TXT_KEY_CITYSTATE_GRANADA_ADJ', 			'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_YELLOW', 		'ARTSTYLE_EUROPEAN', 		'_EURO', 			'EUROPEAN', 		'MINOR_TRAIT_MILITARISTIC',		NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_GWYNEDD', 			'TXT_KEY_CITYSTATE_GWYNEDD', 			'TXT_KEY_CIV5_GWYNEDD_TEXT', 			'TXT_KEY_CITYSTATE_GWYNEDD', 			'TXT_KEY_CITYSTATE_GWYNEDD_ADJ', 			'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_LIGHT_GREEN', 	'ARTSTYLE_EUROPEAN', 		'_EURO', 			'EUROPEAN', 		'MINOR_TRAIT_CULTURED',			NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_HANUABADA', 			'TXT_KEY_CITYSTATE_HANUABADA', 			'TXT_KEY_CIV5_HANUABADA_TEXT', 			'TXT_KEY_CITYSTATE_HANUABADA', 			'TXT_KEY_CITYSTATE_HANUABADA_ADJ', 			'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_LIGHT_PURPLE', 	'ARTSTYLE_POLYNESIAN', 		'_POLYNESIA', 		'ASIAN', 			'MINOR_TRAIT_MERCANTILE',		NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_HONDURAS', 			'TXT_KEY_CITYSTATE_HONDURAS', 			'TXT_KEY_CIV5_HONDURAS_TEXT', 			'TXT_KEY_CITYSTATE_HONDURAS', 			'TXT_KEY_CITYSTATE_HONDURAS_ADJ', 			'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_LIGHT_GREEN', 	'ARTSTYLE_SOUTH_AMERICA', 	'_AMER', 			'AMERICAN', 		'MINOR_TRAIT_MERCANTILE',		NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_HONIARA', 			'TXT_KEY_CITYSTATE_HONIARA', 			'TXT_KEY_CIV5_HONIARA_TEXT', 			'TXT_KEY_CITYSTATE_HONIARA', 			'TXT_KEY_CITYSTATE_HONIARA_ADJ', 			'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_LIGHT_PURPLE', 	'ARTSTYLE_POLYNESIAN', 		'_POLYNESIA', 		'ASIAN', 			'MINOR_TRAIT_MARITIME',			NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_IRUNEA', 			'TXT_KEY_CITYSTATE_IRUNEA', 			'TXT_KEY_CIV5_IRUNEA_TEXT', 			'TXT_KEY_CITYSTATE_IRUNEA', 			'TXT_KEY_CITYSTATE_IRUNEA_ADJ', 			'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_LIGHT_PURPLE', 	'ARTSTYLE_EUROPEAN', 		'_EURO', 			'EUROPEAN', 		'MINOR_TRAIT_MARITIME',			NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_ISKANWAYA', 			'TXT_KEY_CITYSTATE_ISKANWAYA', 			'TXT_KEY_CIV5_ISKANWAYA_TEXT', 			'TXT_KEY_CITYSTATE_ISKANWAYA', 			'TXT_KEY_CITYSTATE_ISKANWAYA_ADJ', 			'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_BLUE', 			'ARTSTYLE_SOUTH_AMERICA', 	'_AMER', 			'AMERICAN', 		'MINOR_TRAIT_RELIGIOUS',		NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_JETARKTE', 			'TXT_KEY_CITYSTATE_JETARKTE', 			'TXT_KEY_CIV5_JETARKTE_TEXT',			'TXT_KEY_CITYSTATE_JETARKTE',			'TXT_KEY_CITYSTATE_JETARKTE_ADJ', 			'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_YELLOW',			'ARTSTYLE_SOUTH_AMERICA', 	'_AMER', 			'AMERICAN', 		'MINOR_TRAIT_MARITIME',			NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_JUYUBIT', 			'TXT_KEY_CITYSTATE_JUYUBIT', 			'TXT_KEY_CIV5_JUYUBIT_TEXT',			'TXT_KEY_CITYSTATE_JUYUBIT',			'TXT_KEY_CITYSTATE_JUYUBIT_ADJ', 			'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_CYAN',			'ARTSTYLE_SOUTH_AMERICA', 	'_AMER', 			'AMERICAN', 		'MINOR_TRAIT_MARITIME',			NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_KAGYRGYN', 			'TXT_KEY_CITYSTATE_KAGYRGYN', 			'TXT_KEY_CIV5_KAGYRGYN_TEXT', 			'TXT_KEY_CITYSTATE_KAGYRGYN', 			'TXT_KEY_CITYSTATE_KAGYRGYN_ADJ', 			'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_PEACH', 			'ARTSTYLE_ASIAN', 			'_ASIA', 			'ASIAN', 			'MINOR_TRAIT_MARITIME',			NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_KARASJOHKA', 		'TXT_KEY_CITYSTATE_KARASJOHKA', 		'TXT_KEY_CIV5_KARASJOHKA_TEXT', 		'TXT_KEY_CITYSTATE_KARASJOHKA', 		'TXT_KEY_CITYSTATE_KARASJOHKA_ADJ', 		'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_PEACH', 			'ARTSTYLE_EUROPEAN', 		'_EURO', 			'EUROPEAN', 		'MINOR_TRAIT_CULTURED',			NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_KARYES', 			'TXT_KEY_CITYSTATE_KARYES', 			'TXT_KEY_CIV5_KARYES_TEXT', 			'TXT_KEY_CITYSTATE_KARYES', 			'TXT_KEY_CITYSTATE_KARYES_ADJ', 			'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_GREEN', 			'ARTSTYLE_GRECO_ROMAN', 	'_EURO', 			'EUROPEAN', 		'MINOR_TRAIT_RELIGIOUS',		NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_KATENDE', 			'TXT_KEY_CITYSTATE_KATENDE', 			'TXT_KEY_CIV5_KATENDE_TEXT', 			'TXT_KEY_CITYSTATE_KATENDE', 			'TXT_KEY_CITYSTATE_KATENDE_ADJ', 			'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_LIGHT_PURPLE', 	'ARTSTYLE_MIDDLE_EAST', 	'_AFRI', 			'AFRICAN', 			'MINOR_TRAIT_CULTURED',			NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_KIGALI', 			'TXT_KEY_CITYSTATE_KIGALI', 			'TXT_KEY_CIV5_KIGALI_TEXT', 			'TXT_KEY_CITYSTATE_KIGALI', 			'TXT_KEY_CITYSTATE_KIGALI_ADJ', 			'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_CYAN', 			'ARTSTYLE_MIDDLE_EAST', 	'_AFRI', 			'AFRICAN', 			'MINOR_TRAIT_MILITARISTIC',		NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_LACONIA', 			'TXT_KEY_CITYSTATE_LACONIA', 			'TXT_KEY_CIV5_LACONIA_TEXT', 			'TXT_KEY_CITYSTATE_LACONIA', 			'TXT_KEY_CITYSTATE_LACONIA_ADJ', 			'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_LIGHT_GREEN', 	'ARTSTYLE_GRECO_ROMAN', 	'_MED', 			'MEDITERRANEAN', 	'MINOR_TRAIT_MILITARISTIC',		NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_LAHORE', 			'TXT_KEY_CITYSTATE_LAHORE', 			'TXT_KEY_CIV5_LAHORE_TEXT', 			'TXT_KEY_CITYSTATE_LAHORE', 			'TXT_KEY_CITYSTATE_LAHORE_ADJ', 			'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_WHITE', 			'ARTSTYLE_MIDDLE_EAST', 	'_ASIA', 			'ASIAN', 			'MINOR_TRAIT_RELIGIOUS',		NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_LEVUKA', 			'TXT_KEY_CITYSTATE_LEVUKA', 			'TXT_KEY_CIV5_LEVUKA_TEXT', 			'TXT_KEY_CITYSTATE_LEVUKA', 			'TXT_KEY_CITYSTATE_LEVUKA_ADJ', 			'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_BLUE', 			'ARTSTYLE_POLYNESIAN', 		'_POLYNESIA', 		'ASIAN', 			'MINOR_TRAIT_MILITARISTIC',		NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_LONGYAN', 			'TXT_KEY_CITYSTATE_LONGYAN', 			'TXT_KEY_CIV5_LONGYAN_TEXT', 			'TXT_KEY_CITYSTATE_LONGYAN', 			'TXT_KEY_CITYSTATE_LONGYAN_ADJ', 			'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_LIGHT_PURPLE', 	'ARTSTYLE_MIDDLE_EAST', 	'_ASIA', 			'ASIAN', 			'MINOR_TRAIT_MILITARISTIC',		NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_MANAGUA', 			'TXT_KEY_CITYSTATE_MANAGUA', 			'TXT_KEY_CIV5_MANAGUA_TEXT', 			'TXT_KEY_CITYSTATE_MANAGUA', 			'TXT_KEY_CITYSTATE_MANAGUA_ADJ', 			'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_PEACH', 			'ARTSTYLE_SOUTH_AMERICA', 	'_AMER', 			'AMERICAN', 		'MINOR_TRAIT_CULTURED',			NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_MATONJENI', 			'TXT_KEY_CITYSTATE_MATONJENI', 			'TXT_KEY_CIV5_MATONJENI_TEXT', 			'TXT_KEY_CITYSTATE_MATONJENI', 			'TXT_KEY_CITYSTATE_MATONJENI_ADJ', 			'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_YELLOW', 		'ARTSTYLE_MIDDLE_EAST', 	'_AFRI', 			'AFRICAN', 			'MINOR_TRAIT_RELIGIOUS',		NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_MBABANE', 			'TXT_KEY_CITYSTATE_MBABANE', 			'TXT_KEY_CIV5_MBABANE_TEXT', 			'TXT_KEY_CITYSTATE_MBABANE', 			'TXT_KEY_CITYSTATE_MBABANE_ADJ', 			'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_WHITE', 			'ARTSTYLE_MIDDLE_EAST', 	'_AFRI', 			'AFRICAN', 			'MINOR_TRAIT_MILITARISTIC',		NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_MENDYARRUP', 		'TXT_KEY_CITYSTATE_MENDYARRUP', 		'TXT_KEY_CIV5_MENDYARRUP_TEXT', 		'TXT_KEY_CITYSTATE_MENDYARRUP', 		'TXT_KEY_CITYSTATE_MENDYARRUP_ADJ', 		'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_MIDDLE_PURPLE', 	'ARTSTYLE_POLYNESIAN', 		'_POLYNESIA', 		'ASIAN', 			'MINOR_TRAIT_MARITIME',			NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_MUISCA', 			'TXT_KEY_CITYSTATE_MUISCA', 			'TXT_KEY_CIV5_MUISCA_TEXT', 			'TXT_KEY_CITYSTATE_MUISCA', 			'TXT_KEY_CITYSTATE_MUISCA_ADJ', 			'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_WHITE', 			'ARTSTYLE_SOUTH_AMERICA', 	'_AMER', 			'AMERICAN', 		'MINOR_TRAIT_MERCANTILE',		NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_MULTAN', 			'TXT_KEY_CITYSTATE_MULTAN', 			'TXT_KEY_CIV5_MULTAN_TEXT', 			'TXT_KEY_CITYSTATE_MULTAN', 			'TXT_KEY_CITYSTATE_MULTAN_ADJ', 			'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_GREEN', 			'ARTSTYLE_MIDDLE_EAST', 	'_ASIA', 			'ASIAN', 			'MINOR_TRAIT_RELIGIOUS',		NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_MUSCAT', 			'TXT_KEY_CITYSTATE_MUSCAT', 			'TXT_KEY_CIV5_MUSCAT_TEXT', 			'TXT_KEY_CITYSTATE_MUSCAT', 			'TXT_KEY_CITYSTATE_MUSCAT_ADJ', 			'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_CYAN', 			'ARTSTYLE_MIDDLE_EAST', 	'_AFRI', 			'AFRICAN', 			'MINOR_TRAIT_MERCANTILE',		NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_MUTITJULU', 			'TXT_KEY_CITYSTATE_MUTITJULU', 			'TXT_KEY_CIV5_MUTITJULU_TEXT', 			'TXT_KEY_CITYSTATE_MUTITJULU', 			'TXT_KEY_CITYSTATE_MUTITJULU_ADJ', 			'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_MIDDLE_PURPLE', 	'ARTSTYLE_POLYNESIAN', 		'_POLYNESIA', 		'ASIAN', 			'MINOR_TRAIT_RELIGIOUS',		NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_NAN_MADOL', 			'TXT_KEY_CITYSTATE_NAN_MADOL', 			'TXT_KEY_CIV5_NAN_MADOL_TEXT', 			'TXT_KEY_CITYSTATE_NAN_MADOL',			'TXT_KEY_CITYSTATE_NAN_MADOL_ADJ', 			'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_PEACH', 			'ARTSTYLE_POLYNESIAN', 		'_POLYNESIA', 		'ASIAN', 			'MINOR_TRAIT_CULTURED',			NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_NGAZARGAMU', 		'TXT_KEY_CITYSTATE_NGAZARGAMU', 		'TXT_KEY_CIV5_NGAZARGAMU_TEXT', 		'TXT_KEY_CITYSTATE_NGAZARGAMU', 		'TXT_KEY_CITYSTATE_NGAZARGAMU_ADJ', 		'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_LIGHT_PURPLE', 	'ARTSTYLE_MIDDLE_EAST', 	'_AFRI', 			'AFRICAN', 			'MINOR_TRAIT_RELIGIOUS',		NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_NYARYANA_MARQ', 		'TXT_KEY_CITYSTATE_NYARYANA_MARQ', 		'TXT_KEY_CIV5_NYARYANA_MARQ_TEXT', 		'TXT_KEY_CITYSTATE_NYARYANA_MARQ', 		'TXT_KEY_CITYSTATE_NYARYANA_MARQ_ADJ', 		'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_GRAY', 			'ARTSTYLE_MIDDLE_EAST', 	'_ASIA', 			'ASIAN', 			'MINOR_TRAIT_CULTURED',			NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_OC_EO', 				'TXT_KEY_CITYSTATE_OC_EO', 				'TXT_KEY_CIV5_OC_EO_TEXT', 				'TXT_KEY_CITYSTATE_OC_EO', 				'TXT_KEY_CITYSTATE_OC_EO_ADJ', 				'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_CYAN', 			'ARTSTYLE_ASIAN', 			'_ASIA', 			'ASIAN', 			'MINOR_TRAIT_MERCANTILE',		NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_ODENSO', 			'TXT_KEY_CITYSTATE_ODENSO', 			'TXT_KEY_CIV5_ODENSO_TEXT', 			'TXT_KEY_CITYSTATE_ODENSO', 			'TXT_KEY_CITYSTATE_ODENSO_ADJ', 			'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_MIDDLE_PURPLE', 	'ARTSTYLE_EUROPEAN', 		'_EURO', 			'EUROPEAN', 		'MINOR_TRAIT_MILITARISTIC',		NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_OUIDAH', 			'TXT_KEY_CITYSTATE_OUIDAH', 			'TXT_KEY_CIV5_OUIDAH_TEXT', 			'TXT_KEY_CITYSTATE_OUIDAH', 			'TXT_KEY_CITYSTATE_OUIDAH_ADJ', 			'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_BLUE', 			'ARTSTYLE_MIDDLE_EAST', 	'_AFRI', 			'AFRICAN', 			'MINOR_TRAIT_MERCANTILE',		NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_PALMYRA', 			'TXT_KEY_CITYSTATE_PALMYRA', 			'TXT_KEY_CIV5_PALMYRA_TEXT', 			'TXT_KEY_CITYSTATE_PALMYRA', 			'TXT_KEY_CITYSTATE_PALMYRA_ADJ', 			'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_PEACH', 			'ARTSTYLE_MIDDLE_EAST', 	'_ASIA', 			'ASIAN', 			'MINOR_TRAIT_MILITARISTIC',		NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_PELYM', 				'TXT_KEY_CITYSTATE_PELYM', 				'TXT_KEY_CIV5_PELYM_TEXT', 				'TXT_KEY_CITYSTATE_PELYM', 				'TXT_KEY_CITYSTATE_PELYM_ADJ', 				'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_CYAN', 			'ARTSTYLE_MIDDLE_EAST', 	'_ASIA', 			'ASIAN', 			'MINOR_TRAIT_MILITARISTIC',		NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_PHANOTEUS', 			'TXT_KEY_CITYSTATE_PHANOTEUS', 			'TXT_KEY_CIV5_PHANOTEUS_TEXT', 			'TXT_KEY_CITYSTATE_PHANOTEUS', 			'TXT_KEY_CITYSTATE_PHANOTEUS_ADJ', 			'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_LIGHT_BLUE', 	'ARTSTYLE_GRECO_ROMAN', 	'_MED', 			'MEDITERRANEAN', 	'MINOR_TRAIT_MERCANTILE',		NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_POKROVKA', 			'TXT_KEY_CITYSTATE_POKROVKA', 			'TXT_KEY_CIV5_POKROVKA_TEXT', 			'TXT_KEY_CITYSTATE_POKROVKA', 			'TXT_KEY_CITYSTATE_POKROVKA_ADJ', 			'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_LIGHT_YELLOW', 	'ARTSTYLE_MIDDLE_EAST', 	'_ASIA', 			'ASIAN', 			'MINOR_TRAIT_MILITARISTIC',		NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_QUELIMANE', 			'TXT_KEY_CITYSTATE_QUELIMANE', 			'TXT_KEY_CIV5_QUELIMANE_TEXT', 			'TXT_KEY_CITYSTATE_QUELIMANE', 			'TXT_KEY_CITYSTATE_QUELIMANE_ADJ', 			'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_PEACH', 			'ARTSTYLE_MIDDLE_EAST', 	'_AFRI', 			'AFRICAN', 			'MINOR_TRAIT_MERCANTILE',		NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_RISHIKESH', 			'TXT_KEY_CITYSTATE_RISHIKESH', 			'TXT_KEY_CIV5_RISHIKESH_TEXT', 			'TXT_KEY_CITYSTATE_RISHIKESH', 			'TXT_KEY_CITYSTATE_RISHIKESH_ADJ', 			'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_GRAY', 			'ARTSTYLE_MIDDLE_EAST', 	'_ASIA', 			'ASIAN', 			'MINOR_TRAIT_RELIGIOUS',		NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_SADDARVAZEH', 		'TXT_KEY_CITYSTATE_SADDARVAZEH', 		'TXT_KEY_CIV5_SADDARVAZEH_TEXT', 		'TXT_KEY_CITYSTATE_SADDARVAZEH',		'TXT_KEY_CITYSTATE_SADDARVAZEH_ADJ', 		'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_PEACH', 			'ARTSTYLE_MIDDLE_EAST', 	'_ASIA', 			'ASIAN', 			'MINOR_TRAIT_RELIGIOUS',		NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_SANAA', 				'TXT_KEY_CITYSTATE_SANAA', 				'TXT_KEY_CIV5_SANAA_TEXT', 				'TXT_KEY_CITYSTATE_SANAA', 				'TXT_KEY_CITYSTATE_SANAA_ADJ', 				'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_LIGHT_YELLOW', 	'ARTSTYLE_MIDDLE_EAST', 	'_AFRI', 			'AFRICAN', 			'MINOR_TRAIT_MERCANTILE',		NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_SANANDAJ', 			'TXT_KEY_CITYSTATE_SANANDAJ', 			'TXT_KEY_CIV5_SANANDAJ_TEXT', 			'TXT_KEY_CITYSTATE_SANANDAJ', 			'TXT_KEY_CITYSTATE_SANANDAJ_ADJ', 			'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_WHITE', 			'ARTSTYLE_EUROPEAN', 		'_ASIA', 			'ASIAN', 			'MINOR_TRAIT_CULTURED',			NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_SANTO_DOMINGO', 		'TXT_KEY_CITYSTATE_SANTO_DOMINGO', 		'TXT_KEY_CIV5_SANTO_DOMINGO_TEXT', 		'TXT_KEY_CITYSTATE_SANTO_DOMINGO', 		'TXT_KEY_CITYSTATE_SANTO_DOMINGO_ADJ', 		'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_LIGHT_BLUE', 	'ARTSTYLE_EUROPEAN', 		'_EURO', 			'EUROPEAN', 		'MINOR_TRAIT_MERCANTILE',		NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_SARNATH', 			'TXT_KEY_CITYSTATE_SARNATH', 			'TXT_KEY_CIV5_SARNATH_TEXT', 			'TXT_KEY_CITYSTATE_SARNATH', 			'TXT_KEY_CITYSTATE_SARNATH_ADJ', 			'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_LIGHT_YELLOW', 	'ARTSTYLE_MIDDLE_EAST', 	'_ASIA', 			'ASIAN', 			'MINOR_TRAIT_RELIGIOUS',		NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_SGANG_GWAAY', 		'TXT_KEY_CITYSTATE_SGANG_GWAAY', 		'TXT_KEY_CIV5_SGANG_GWAAY_TEXT',		'TXT_KEY_CITYSTATE_SGANG_GWAAY',		'TXT_KEY_CITYSTATE_SGANG_GWAAY_ADJ', 		'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_LIGHT_GREEN', 	'ARTSTYLE_SOUTH_AMERICA', 	'_AMER', 			'AMERICAN', 		'MINOR_TRAIT_MARITIME',			NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_SIERRA_LEONE', 		'TXT_KEY_CITYSTATE_SIERRA_LEONE', 		'TXT_KEY_CIV5_SIERRA_LEONE_TEXT',		'TXT_KEY_CITYSTATE_SIERRA_LEONE',		'TXT_KEY_CITYSTATE_SIERRA_LEONE_ADJ', 		'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_MIDDLE_BLUE', 	'ARTSTYLE_MIDDLE_EAST', 	'_AFRI', 			'AFRICAN', 			'MINOR_TRAIT_CULTURED',			NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_SUCEAVA', 			'TXT_KEY_CITYSTATE_SUCEAVA', 			'TXT_KEY_CIV5_SUCEAVA_TEXT', 			'TXT_KEY_CITYSTATE_SUCEAVA', 			'TXT_KEY_CITYSTATE_SUCEAVA_ADJ', 			'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_MIDDLE_PURPLE', 	'ARTSTYLE_EUROPEAN', 		'_EURO', 			'EUROPEAN', 		'MINOR_TRAIT_RELIGIOUS',		NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_SURREY', 			'TXT_KEY_CITYSTATE_SURREY', 			'TXT_KEY_CIV5_SURREY_TEXT', 			'TXT_KEY_CITYSTATE_SURREY', 			'TXT_KEY_CITYSTATE_SURREY_ADJ', 			'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_GRAY', 			'ARTSTYLE_EUROPEAN', 		'_EURO', 			'EUROPEAN', 		'MINOR_TRAIT_MILITARISTIC',		NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_TAIWAN', 			'TXT_KEY_CITYSTATE_TAIWAN', 			'TXT_KEY_CIV5_TAIWAN_TEXT', 			'TXT_KEY_CITYSTATE_TAIWAN', 			'TXT_KEY_CITYSTATE_TAIWAN_ADJ', 			'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_GREEN', 			'ARTSTYLE_ASIAN', 			'_ASIA', 			'ASIAN', 			'MINOR_TRAIT_MARITIME',			NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_TBILISI', 			'TXT_KEY_CITYSTATE_TBILISI', 			'TXT_KEY_CIV5_TBILISI_TEXT', 			'TXT_KEY_CITYSTATE_TBILISI', 			'TXT_KEY_CITYSTATE_TBILISI_ADJ', 			'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_GRAY', 			'ARTSTYLE_MIDDLE_EAST', 	'_ASIA', 			'ASIAN', 			'MINOR_TRAIT_MILITARISTIC',		NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_THIMPHU', 			'TXT_KEY_CITYSTATE_THIMPHU', 			'TXT_KEY_CIV5_THIMPHU_TEXT', 			'TXT_KEY_CITYSTATE_THIMPHU', 			'TXT_KEY_CITYSTATE_THIMPHU_ADJ', 			'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_MIDDLE_BLUE', 	'ARTSTYLE_ASIAN', 			'_ASIA', 			'ASIAN', 			'MINOR_TRAIT_CULTURED',			NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_TIWANAKU', 			'TXT_KEY_CITYSTATE_TIWANAKU', 			'TXT_KEY_CIV5_TIWANAKU_TEXT', 			'TXT_KEY_CITYSTATE_TIWANAKU', 			'TXT_KEY_CITYSTATE_TIWANAKU_ADJ', 			'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_LIGHT_YELLOW', 	'ARTSTYLE_SOUTH_AMERICA', 	'_AMER', 			'AMERICAN', 		'MINOR_TRAIT_RELIGIOUS',		NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_TUNIS', 				'TXT_KEY_CITYSTATE_TUNIS', 				'TXT_KEY_CIV5_TUNIS_TEXT', 				'TXT_KEY_CITYSTATE_TUNIS', 				'TXT_KEY_CITYSTATE_TUNIS_ADJ', 				'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_GRAY', 			'ARTSTYLE_MIDDLE_EAST', 	'_AFRI', 			'AFRICAN', 			'MINOR_TRAIT_MERCANTILE',		NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_VADUZ', 				'TXT_KEY_CITYSTATE_VADUZ', 				'TXT_KEY_CIV5_VADUZ_TEXT', 				'TXT_KEY_CITYSTATE_VADUZ', 				'TXT_KEY_CITYSTATE_VADUZ_ADJ', 				'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_GREEN', 			'ARTSTYLE_EUROPEAN', 		'_EURO', 			'EUROPEAN', 		'MINOR_TRAIT_CULTURED',			NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_WOOTEI_NIICIE', 		'TXT_KEY_CITYSTATE_WOOTEI_NIICIE', 		'TXT_KEY_CIV5_WOOTEI_NIICIE_TEXT',		'TXT_KEY_CITYSTATE_WOOTEI_NIICIE',		'TXT_KEY_CITYSTATE_WOOTEI_NIICIE_ADJ', 		'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_LIGHT_GREEN', 	'ARTSTYLE_SOUTH_AMERICA', 	'_AMER', 			'AMERICAN', 		'MINOR_TRAIT_MILITARISTIC',		NULL,				1,			'UNITCLASS_WORKER' UNION ALL
SELECT 		'MINOR_CIV_YANGCHENG', 			'TXT_KEY_CITYSTATE_YANGCHENG', 			'TXT_KEY_CIV5_YANGCHENG_TEXT', 			'TXT_KEY_CITYSTATE_YANGCHENG', 			'TXT_KEY_CITYSTATE_YANGCHENG_ADJ', 			'ART_DEF_CIVILIZATION_MINOR', 	'PLAYERCOLOR_MINOR_GRAY', 			'ARTSTYLE_ASIAN', 			'_ASIA', 			'ASIAN', 			'MINOR_TRAIT_RELIGIOUS',		NULL,				1,			'UNITCLASS_WORKER';
...
SELECT 		'MINOR_CIV_BAGHCASARAY', 		'TXT_KEY_CITYSTATE_BAGHCASARAY' UNION ALL
SELECT 		'MINOR_CIV_BALKH', 				'TXT_KEY_CITYSTATE_BALKH' UNION ALL
SELECT 		'MINOR_CIV_BALLEY_CHASHTAL', 	'TXT_KEY_CITYSTATE_BALLEY_CHASHTAL' UNION ALL
...
SELECT 		'MINOR_CIV_KAGYRGYN', 			'TXT_KEY_CITYSTATE_KAGYRGYN' UNION ALL
...
SELECT 		'MINOR_CIV_MATONJENI', 			'TXT_KEY_CITYSTATE_MATONJENI' UNION ALL
...
SELECT 		'MINOR_CIV_NGAZARGAMU', 		'TXT_KEY_CITYSTATE_NGAZARGAMU' UNION ALL
...
SELECT 		'MINOR_CIV_SANANDAJ', 			'TXT_KEY_CITYSTATE_SANANDAJ' UNION ALL
...
SELECT 		'MINOR_CIV_VADUZ', 				'TXT_KEY_CITYSTATE_VADUZ' UNION ALL
...
SELECT 		'MINOR_CIV_MENDYARRUP', 		'FLAVOR_OFFENSE',			5 UNION ALL
SELECT 		'MINOR_CIV_VADUZ', 				'FLAVOR_GOLD',				8 UNION ALL
SELECT 		'MINOR_CIV_VADUZ', 				'FLAVOR_CULTURE',			7 UNION ALL
SELECT 		'MINOR_CIV_KAGYRGYN', 			'FLAVOR_DEFENSE',			8 UNION ALL
SELECT 		'MINOR_CIV_KAGYRGYN', 			'FLAVOR_GROWTH',			5 UNION ALL
SELECT 		'MINOR_CIV_KAGYRGYN', 			'FLAVOR_OFFENSE',			2 UNION ALL
SELECT 		'MINOR_CIV_MATONJENI', 			'FLAVOR_RELIGION',			9 UNION ALL
SELECT 		'MINOR_CIV_MATONJENI', 			'FLAVOR_GROWTH',			6 UNION ALL
SELECT 		'MINOR_CIV_NGAZARGAMU', 		'FLAVOR_GOLD',				6 UNION ALL
SELECT 		'MINOR_CIV_NGAZARGAMU', 		'FLAVOR_EXPANSION',			6 UNION ALL
SELECT 		'MINOR_CIV_NGAZARGAMU', 		'FLAVOR_RELIGION',			3 UNION ALL
SELECT 		'MINOR_CIV_BALLEY_CHASHTAL', 	'FLAVOR_CULTURE',			7 UNION ALL
SELECT 		'MINOR_CIV_BALLEY_CHASHTAL', 	'FLAVOR_GROWTH',			7 UNION ALL
SELECT 		'MINOR_CIV_BALLEY_CHASHTAL', 	'FLAVOR_CITY_DEFENSE',		1 UNION ALL
SELECT 		'MINOR_CIV_SANANDAJ', 			'FLAVOR_CULTURE',			10 UNION ALL
SELECT 		'MINOR_CIV_SANANDAJ', 			'FLAVOR_EXPANSION',			5 UNION ALL
SELECT 		'MINOR_CIV_BAGHCASARAY', 		'FLAVOR_MILITARY_TRAINING',	7 UNION ALL
SELECT 		'MINOR_CIV_BAGHCASARAY', 		'FLAVOR_DEFENSE',			4 UNION ALL
SELECT 		'MINOR_CIV_BAGHCASARAY', 		'FLAVOR_EXPANSION',			4;
...
INSERT INTO MajorBlocksMinor
			(MajorCiv, 						MinorCiv)
SELECT 		'CIVILIZATION_GT_BASQUES',		'MINOR_CIV_IRUNEA'		WHERE EXISTS (SELECT * FROM BuildingClasses WHERE Type='BUILDINGCLASS_DUMMY_UCS_BASQUE') UNION ALL
SELECT 		'CIVILIZATION_JAR_NANMADOL',	'MINOR_CIV_NAN_MADOL'	WHERE EXISTS (SELECT * FROM BuildingClasses WHERE Type='BUILDINGCLASS_DUMMY_UCS_NAN_MADOL') /*UNION ALL
SELECT 		'CIVILIZATION_JAR_MARAJOARA',	'MINOR_CIV_CAMUTINS'	WHERE EXISTS (SELECT * FROM BuildingClasses WHERE Type='BUILDINGCLASS_DUMMY_UCS_MARAJOARA')*/;
---------------------------------------------------
...
SELECT 'POLICY_BALLEY_CHASHTAL', 		'TXT_KEY_CITYSTATE_BALLEY_CHASHTAL' UNION ALL
...
SELECT 'POLICY_VADUZ', 					'TXT_KEY_CITYSTATE_VADUZ' UNION ALL
...
INSERT INTO Policy_GreatWorkYieldChanges 
			(PolicyType, 				YieldType, 			Yield)
SELECT 		'POLICY_BALLEY_CHASHTAL',	'YIELD_FOOD', 		1 UNION ALL
SELECT 		'POLICY_BALLEY_CHASHTAL',	'YIELD_CULTURE', 	1 UNION ALL
SELECT 		'POLICY_BUCHAREST',			'YIELD_CULTURE', 	1 UNION ALL
SELECT 		'POLICY_VADUZ',				'YIELD_GOLD', 		1;
...
SELECT 		'PROMOTION_BAGHCASARAY', 					'TXT_KEY_PROMOTION_BAGHCASARAY', 			'TXT_KEY_PROMOTION_BAGHCASARAY_HELP', 			1, 				'AS2D_IF_LEVELUP', 	53, 			'UCS_PROMOTION_ATLAS', 	'PEDIA_SHARED', 	'TXT_KEY_PROMOTION_BAGHCASARAY' UNION ALL
...
SELECT 		'PROMOTION_BAGHCASARAY', 			'UNITCOMBAT_MOUNTED' UNION ALL
SELECT 		'PROMOTION_BAGHCASARAY', 			'UNITCOMBAT_ARMOR' UNION ALL
...
SELECT 		'BUILDINGCLASS_BALLEY_CHASHTAL', 		'BUILDING_BALLEY_CHASHTAL',	 		'TXT_KEY_CITYSTATE_BALLEY_CHASHTAL' UNION ALL
...
SELECT 		'BUILDINGCLASS_KAGYRGYN', 				'BUILDING_KAGYRGYN', 				'TXT_KEY_CITYSTATE_KAGYRGYN' UNION ALL
...
SELECT 		'BUILDINGCLASS_MATONJENI', 				'BUILDING_MATONJENI',	 			'TXT_KEY_CITYSTATE_MATONJENI' UNION ALL
...
SELECT 		'BUILDINGCLASS_SANANDAJ', 				'BUILDING_SANANDAJ', 				'TXT_KEY_CITYSTATE_SANANDAJ' UNION ALL
...
SELECT 		'BUILDING_BALLEY_CHASHTAL', 		'BUILDINGCLASS_BALLEY_CHASHTAL', 		-1, 	0, 					NULL, 		'TXT_KEY_CITYSTATE_BALLEY_CHASHTAL', 		'TXT_KEY_CSTRAIT_MINOR_CIV_BALLEY_CHASHTAL', 	-1, 			1, 			0, 				1, 				5, 					'UCS_BUILDING_ATLAS', 			1, 				1,			-1 UNION ALL
...
SELECT 		'BUILDING_KAGYRGYN', 				'BUILDINGCLASS_KAGYRGYN', 				-1, 	0, 					NULL, 		'TXT_KEY_CITYSTATE_KAGYRGYN', 				'TXT_KEY_CSTRAIT_MINOR_CIV_KAGYRGYN', 			-1, 			1, 			0, 				1, 				5, 					'UCS_BUILDING_ATLAS', 			1, 				1,			-1 UNION ALL
...
SELECT 		'BUILDING_MATONJENI', 				'BUILDINGCLASS_MATONJENI', 				-1, 	0, 					NULL, 		'TXT_KEY_CITYSTATE_MATONJENI', 				'TXT_KEY_CSTRAIT_MINOR_CIV_MATONJENI', 			-1, 			1, 			0, 				1, 				5, 					'UCS_BUILDING_ATLAS', 			1, 				1,			-1 UNION ALL
...
SELECT 		'BUILDING_SANANDAJ', 				'BUILDINGCLASS_SANANDAJ', 				-1, 	0, 					NULL, 		'TXT_KEY_CITYSTATE_SANANDAJ', 				'TXT_KEY_CSTRAIT_MINOR_CIV_SANANDAJ', 			-1, 			1, 			0, 				1, 				5, 					'UCS_BUILDING_ATLAS', 			1, 				1,			-1 UNION ALL
...
UPDATE Buildings SET WonderSplashImage = 'UCS_Potala_Palace_splash.dds', WonderSplashAudio = 'AS2D_WONDER_LHASA', WonderSplashAnchor = 'L,B', Quote = 'TXT_KEY_BUILDING_LHASA_QUOTE', Civilopedia = 'TXT_KEY_BUILDING_LHASA_PEDIA', SingleLeagueVotes = 1, DistressFlatReduction = 2, PrereqTech = 'TECH_ARCHITECTURE', EventChoiceRequiredActive = 'PLAYER_EVENT_CHOICE_MINOR_CIV_LHASA' WHERE Type = 'BUILDING_LHASA';
...
UPDATE Buildings SET ExtraCityHitPoints = 25, Water = 1, MinAreaSize = 10, EventChoiceRequiredActive = 'PLAYER_EVENT_CHOICE_MINOR_CIV_VALLETTA' WHERE Type = 'BUILDING_VALLETTA';
...
INSERT INTO Building_YieldChanges
			(BuildingType, 			YieldType, 							Yield)
SELECT 		'BUILDING_LHASA', 		'YIELD_CULTURE', 					3 UNION ALL
...
SELECT 		'BUILDING_LHASA', 			'YIELD_FAITH', 			5 UNION ALL
...
SELECT		'BUILDING_KAGYRGYN',	'GREATPERSON_GENERAL',	'YIELD_FOOD',	'YIELD_FOOD',	75 UNION ALL
SELECT		'BUILDING_MATONJENI',	'GREATPERSON_PROPHET',	'YIELD_FOOD',	'YIELD_FOOD',	125;

INSERT INTO Building_YieldChangesPerLocalTheme
			(BuildingType,				YieldType,		Yield)
SELECT		'BUILDING_BALLEY_CHASHTAL',	'YIELD_FOOD',	10;
...
SELECT 		'BUILDING_SANANDAJ',	'UNIT_ENVOY', 		1 UNION ALL
...
SELECT  	'PLAYER_EVENT_MINOR_CIV_BAGHCASARAY', 		'TXT_KEY_CSTRAIT_MINOR_CIV', 	'TXT_KEY_CSTRAIT_MINOR_CIV_BAGHCASARAY', 		'AllianceShift.dds', -1, 1, 1, 1 UNION ALL
...
SELECT  	'PLAYER_EVENT_MINOR_CIV_BALLEY_CHASHTAL', 	'TXT_KEY_CSTRAIT_MINOR_CIV', 	'TXT_KEY_CSTRAIT_MINOR_CIV_BALLEY_CHASHTAL', 	'AllianceShift.dds', -1, 1, 1, 1 UNION ALL
...
SELECT  	'PLAYER_EVENT_MINOR_CIV_KAGYRGYN', 			'TXT_KEY_CSTRAIT_MINOR_CIV', 	'TXT_KEY_CSTRAIT_MINOR_CIV_KAGYRGYN', 			'AllianceShift.dds', -1, 1, 1, 1 UNION ALL
...
SELECT  	'PLAYER_EVENT_MINOR_CIV_MATONJENI', 		'TXT_KEY_CSTRAIT_MINOR_CIV', 	'TXT_KEY_CSTRAIT_MINOR_CIV_MATONJENI', 			'AllianceShift.dds', -1, 1, 1, 1 UNION ALL
...
SELECT  	'PLAYER_EVENT_MINOR_CIV_NGAZARGAMU', 		'TXT_KEY_CSTRAIT_MINOR_CIV', 	'TXT_KEY_CSTRAIT_MINOR_CIV_NGAZARGAMU', 		'AllianceShift.dds', -1, 1, 1, 1 UNION ALL
...
SELECT  	'PLAYER_EVENT_MINOR_CIV_SANANDAJ', 			'TXT_KEY_CSTRAIT_MINOR_CIV', 	'TXT_KEY_CSTRAIT_MINOR_CIV_SANANDAJ', 			'AllianceShift.dds', -1, 1, 1, 1 UNION ALL
...
SELECT  	'PLAYER_EVENT_MINOR_CIV_VADUZ', 			'TXT_KEY_CSTRAIT_MINOR_CIV', 	'TXT_KEY_CSTRAIT_MINOR_CIV_VADUZ', 				'AllianceShift.dds', -1, 1, 1, 1 UNION ALL
...
SELECT 		'PLAYER_EVENT_CHOICE_MINOR_CIV_BAGHCASARAY', 		'TXT_KEY_CITYSTATE_BAGHCASARAY', 		'TXT_KEY_CSTRAIT_MINOR_CIV_BAGHCASARAY', 		3, 				1, 			0,					NULL, 						'PROMOTION_BAGHCASARAY', 	NULL UNION ALL
...
SELECT 		'PLAYER_EVENT_CHOICE_MINOR_CIV_BALLEY_CHASHTAL', 	'TXT_KEY_CITYSTATE_BALLEY_CHASHTAL', 	'TXT_KEY_CSTRAIT_MINOR_CIV_BALLEY_CHASHTAL', 	3, 				1, 			0,					'POLICY_BALLEY_CHASHTAL', 	NULL, 						'BUILDINGCLASS_BALLEY_CHASHTAL' UNION ALL
...
SELECT 		'PLAYER_EVENT_CHOICE_MINOR_CIV_KAGYRGYN', 			'TXT_KEY_CITYSTATE_KAGYRGYN', 			'TXT_KEY_CSTRAIT_MINOR_CIV_KAGYRGYN', 			3, 				1, 			0,					NULL, 						NULL, 						'BUILDINGCLASS_KAGYRGYN' UNION ALL
...
SELECT 		'PLAYER_EVENT_CHOICE_MINOR_CIV_MATONJENI', 			'TXT_KEY_CITYSTATE_MATONJENI', 			'TXT_KEY_CSTRAIT_MINOR_CIV_MATONJENI', 			3, 				1, 			0,					NULL, 						NULL, 						'BUILDINGCLASS_MATONJENI' UNION ALL
...
SELECT 		'PLAYER_EVENT_CHOICE_MINOR_CIV_NGAZARGAMU', 		'TXT_KEY_CITYSTATE_NGAZARGAMU', 		'TXT_KEY_CSTRAIT_MINOR_CIV_NGAZARGAMU', 		3, 				1, 			0,					NULL, 						NULL, 						NULL UNION ALL
...
SELECT 		'PLAYER_EVENT_CHOICE_MINOR_CIV_SANANDAJ', 			'TXT_KEY_CITYSTATE_SANANDAJ', 			'TXT_KEY_CSTRAIT_MINOR_CIV_SANANDAJ', 			3, 				1, 			0,					NULL, 						NULL, 						NULL UNION ALL
...
SELECT 		'PLAYER_EVENT_CHOICE_MINOR_CIV_VADUZ', 				'TXT_KEY_CITYSTATE_VADUZ', 				'TXT_KEY_CSTRAIT_MINOR_CIV_VADUZ', 				3, 				1, 			0,					'POLICY_VADUZ', 			NULL, 						NULL UNION ALL
...
SELECT 		'PLAYER_EVENT_CHOICE_MINOR_CIV_BAGHCASARAY', 		'PLAYER_EVENT_MINOR_CIV_BAGHCASARAY' UNION ALL
SELECT 		'PLAYER_EVENT_CHOICE_MINOR_CIV_BALKH', 				'PLAYER_EVENT_MINOR_CIV_BALKH' UNION ALL
SELECT 		'PLAYER_EVENT_CHOICE_MINOR_CIV_BALLEY_CHASHTAL', 	'PLAYER_EVENT_MINOR_CIV_BALLEY_CHASHTAL' UNION ALL
...
SELECT 		'PLAYER_EVENT_CHOICE_MINOR_CIV_KAGYRGYN', 			'PLAYER_EVENT_MINOR_CIV_KAGYRGYN' UNION ALL
...
SELECT 		'PLAYER_EVENT_CHOICE_MINOR_CIV_MATONJENI', 			'PLAYER_EVENT_MINOR_CIV_MATONJENI' UNION ALL
...
SELECT 		'PLAYER_EVENT_CHOICE_MINOR_CIV_NGAZARGAMU', 		'PLAYER_EVENT_MINOR_CIV_NGAZARGAMU' UNION ALL
...
SELECT 		'PLAYER_EVENT_CHOICE_MINOR_CIV_SANANDAJ', 			'PLAYER_EVENT_MINOR_CIV_SANANDAJ' UNION ALL
...
SELECT 		'PLAYER_EVENT_CHOICE_MINOR_CIV_VADUZ', 				'PLAYER_EVENT_MINOR_CIV_VADUZ' UNION ALL
...
SELECT 		'PLAYER_EVENT_CHOICE_MINOR_CIV_LHASA', 			'BUILDINGCLASS_LHASA', 			'YIELD_FAITH', 		5 UNION ALL
...
SELECT 		'PLAYER_EVENT_CHOICE_MINOR_CIV_KAGYRGYN', 		'IMPROVEMENT_CAMP', 				'YIELD_GREAT_GENERAL_POINTS', 	1 UNION ALL
SELECT 		'PLAYER_EVENT_CHOICE_MINOR_CIV_KAGYRGYN', 		'IMPROVEMENT_FISHING_BOATS', 		'YIELD_GREAT_GENERAL_POINTS', 	1 UNION ALL
...
SELECT 		'PLAYER_EVENT_CHOICE_MINOR_CIV_MATONJENI', 		'IMPROVEMENT_HOLY_SITE', 			'YIELD_FOOD', 					4 UNION ALL
...
SELECT 		'PLAYER_EVENT_CHOICE_MINOR_CIV_NGAZARGAMU', 	'IMPROVEMENT_TRADING_POST', 		'YIELD_PRODUCTION', 			1 UNION ALL
SELECT 		'PLAYER_EVENT_CHOICE_MINOR_CIV_NGAZARGAMU', 	'IMPROVEMENT_CUSTOMS_HOUSE', 		'YIELD_PRODUCTION', 			1 UNION ALL
...
INSERT INTO Language_en_US
			(Tag, 										Text)
SELECT		'TXT_KEY_RESOURCE_GLASS_MONOPOLY',			'[COLOR_POSITIVE_TEXT]Monopoly Bonus:[ENDCOLOR] Each currently applied Happiness or flat-yield [ICON_MONOPOLY] Global Monopoly modifier gains an additional +1 bonus, and percentage-based [ICON_MONOPOLY] Global Monopoly modifier gains an additional +1% bonus.' UNION ALL
...
UPDATE Language_en_US SET Text = REPLACE(Text, 'Requires an access to [COLOR_CYAN]Lhasa[ENDCOLOR]''s ability and an active session of [COLOR_POSITIVE_TEXT]World Congress[ENDCOLOR].[NEWLINE][NEWLINE]', '') WHERE Tag ='TXT_KEY_BUILDING_LHASA_HELP' AND EXISTS (SELECT * FROM BuildingClasses WHERE Type='BUILDINGCLASS_DUMMY_UCS_MW');

INSERT INTO Language_en_US (Tag, Text) 
SELECT 'TXT_KEY_BUILDING_LHASA_HELP_CUT', 'Majestic [COLOR_YIELD_FOOD]Potala Palace[ENDCOLOR] is a residence of Dalai Lama (+3[ICON_CULTURE]; +10%[ICON_CULTURE]; +10%[ICON_PEACE]; modifiers are [COLOR_NEGATIVE_TEXT]halved[ENDCOLOR] when not allied to [COLOR_CYAN]Lhasa[ENDCOLOR]) and surprisingly it also serves as a huge residential building (-2[ICON_HAPPINESS_3] Distress). It strongly ties your empire with a small peaceful nation ([COLOR_YIELD_GOLD]Sphere of Influence[ENDCOLOR] over [COLOR_CYAN]Lhasa[ENDCOLOR]). Voice of Dalai Lama is not silent after his exile and still tries to resolve the conflict without any violence (+1[ICON_DIPLOMAT] League Vote).'
WHERE EXISTS (SELECT * FROM Community WHERE Type='BUILDINGCLASS_DUMMY_UCS_MW');
				
UPDATE Language_en_US SET Text = 'Special: [COLOR_WHITE]Lhasa''s ability[ENDCOLOR], [COLOR:45:150:50:255]World Congress[ENDCOLOR].[NEWLINE][NEWLINE]'||Text WHERE Tag ='TXT_KEY_BUILDING_LHASA_HELP' AND EXISTS (SELECT * FROM Community WHERE Type='BUILDINGCLASS_DUMMY_UCS_MW');
--============================================--
...
UPDATE Units SET CombatClass = 'UNITCOMBAT_SIEGE'			WHERE Type ='UNIT_SIEGE_TOWER';
